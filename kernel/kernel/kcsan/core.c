...
static bool kcsan_early_enable = false;
unsigned int kcsan_udelay_task = CONFIG_KCSAN_UDELAY_TASK;
unsigned int kcsan_udelay_interrupt = CONFIG_KCSAN_UDELAY_INTERRUPT;
static long kcsan_skip_watch = CONFIG_KCSAN_SKIP_WATCH;
static bool kcsan_interrupt_watcher = false;
...
static PER_CPU_VAR(struct kcsan_ctx kcsan_cpu_ctx) = {
	.scoped_accesses	= {LIST_REMOVE_POISON, NULL},
};
 ...
 */
static PER_CPU_VAR(long kcsan_skip);
...
static noinline void kcsan_check_scoped_accesses(void)
{
	...
	ctx->disable_scoped++;
	list_for_every(&ctx->scoped_accesses) {
		scoped_access = container_of(l, struct kcsan_scoped_access, list);
...
should_watch(struct kcsan_ctx *ctx, const volatile void *ptr, size_t size, int type)
{
	...
	dec_per_cpu(kcsan_skip);

	if (get_per_cpu(kcsan_skip) >= 0)
		return false;
...
{
	u32 state = get_per_cpu(kcsan_rand_state);

	state = 1664525 * state + 1013904223;
	write_per_cpu(kcsan_rand_state, state);
...
static inline void reset_kcsan_skip(void)
{
	long skip_count = kcsan_skip_watch -
			  (/*IS_ENABLED(CONFIG_KCSAN_SKIP_WATCH_RANDOMIZE)*/ 0 ?
				   kcsan_prandom_u32_max(kcsan_skip_watch) :
				   0);
	write_per_cpu(kcsan_skip, skip_count);
}
...
{
	unsigned int delay = !is_in_interrupt() ? kcsan_udelay_task : kcsan_udelay_interrupt;
	/* For certain access types, skew the random delay to be longer. */
	unsigned int skew_delay_order =
		(type & (KCSAN_ACCESS_COMPOUND | KCSAN_ACCESS_ASSERT)) ? 1 : 0;

	delay -= /*IS_ENABLED(CONFIG_KCSAN_DELAY_RANDOMIZE)*/ 0 ?
...
kcsan_setup_watchpoint(const volatile void *ptr, size_t size, int type, unsigned long ip)
{
	...
	if (!consume_watchpoint(watchpoint)) {
		...
		if (/*IS_ENABLED(CONFIG_KCSAN_REPORT_RACE_UNKNOWN_ORIGIN)*/ 1 || is_assert) {
			  ...
			  struct kcsan_scoped_access *sa)
{
	...
	check_access(ptr, size, type, (unsigned long) __builtin_return_address(0));
	...
	sa->size = size;
	sa->type = type;
	sa->ip = (unsigned long) __builtin_return_address(0);
...
void kcsan_end_scoped_access(struct kcsan_scoped_access *sa)
{
	...
	list_remove(&sa->list);
	if (list_is_empty(&ctx->scoped_accesses))
		/*
...
#define DEFINE_TSAN_VOLATILE_READ_WRITE(size)                                  \
	...
	{                                                                      \
		...
		if (CONFIG_KCSAN_IGNORE_ATOMICS && is_atomic)      \
			return;                                                \
...
#define DEFINE_TSAN_ATOMIC_LOAD_STORE(bits)                                                        \
	...
	{                                                                                          \
		...
		if (!CONFIG_KCSAN_IGNORE_ATOMICS) {                                    \
