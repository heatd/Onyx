...
const uint16_t subsystems[] = {virtio::network_pci_subsys, virtio::block_pci_subsys};
...
bool vdev::find_structures()
{
    ...
    while ((cap_off = dev->find_capability(PCI_CAP_ID_VENDOR, instance_nr)) != 0)
    {
        ...
        {
            structure->notify_off_mult =
                dev->read(cap_off + virtio::notify_off_multiplier, sizeof(uint32_t));
        }
...
bool virtq_split::init()
{
    ...
    size_t used_ring_length = queue_size * sizeof(virtq_used_elem) + sizeof(uint16_t) * 2;
    size_t total_pages =
    ...
    eff_queue_notify_off =
        (multiplier * device->read_config<uint16_t>(pci_common_cfg::queue_notify_off));
...
{
    // Ergh, using spin_lock_irqrestore here is tough since we need to coordinate with
    // wait_for_event_locked
    ...
    if (!irq_context)
    {
        wait_for_event_locked(&desc_alloc_wq, has_available_descriptors(nr_descs),
                              &desc_alloc_lock);
    }
    else
    {
...
void virtq_split::allocate_buffer_list(virtio_allocation_info &info)
{
    ...
    for (size_t i = 0; i < info.nr_vecs; i++, vec++)
    {
        ...
        if (seq - 1 == 0)
            completions[index] = info.completion;
...
struct pci::pci_id virtio_pci_ids[] = {{PCI_ID_DEVICE(VIRTIO_VENDOR_ID, PCI_ANY_ID, NULL)},
...
struct driver virtio_driver = {.name = "virtio",
                               .devids = &virtio_pci_ids,
                               .probe = virtio_probe,
                               .bus_type_node = {&virtio_driver}};

extern "C" int virtio_init(void)
{
