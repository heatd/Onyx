...
acpi_rs_dump_address_common (
	union acpi_resource_data        *resource);
		...
		if (type == ACPI_RESOURCE_TYPE_SERIAL_BUS) {
			acpi_rs_dump_descriptor (&resource_list->data,
				acpi_gbl_dump_serial_bus_dispatch[
					resource_list->data.common_serial_bus.type]);
		}
		else {
			acpi_rs_dump_descriptor (&resource_list->data,
	...
	struct acpi_rsdump_info         *table)
{
	...
	while (count) {
		...
		case ACPI_RSD_LITERAL:
			...
			acpi_rs_out_string (name, ACPI_CAST_PTR (char, table->pointer));
			break;
		...
		case ACPI_RSD_1BITFLAG:
			...
			acpi_rs_out_string (name, table->pointer [*target & 0x01]);
			break;

		case ACPI_RSD_2BITFLAG:

			acpi_rs_out_string (name, table->pointer [*target & 0x03]);
			break;

		case ACPI_RSD_3BITFLAG:

			acpi_rs_out_string (name, table->pointer [*target & 0x07]);
			break;
		...
		case ACPI_RSD_SHORTLIST:
			...
			if (previous_target) {
				...
				acpi_rs_dump_short_byte_list (*previous_target, target);
			}
			break;
		...
		case ACPI_RSD_SHORTLISTX:
			...
			if (previous_target) {
				...
				acpi_rs_dump_short_byte_list (*previous_target,
					*(ACPI_CAST_INDIRECT_PTR (u8, target)));
			}
			break;
		...
		case ACPI_RSD_LONGLIST:
			 ...
			 */
			if (previous_target) {
				acpi_rs_dump_byte_list (ACPI_GET16 (previous_target), target);
			}
			break;
		...
		case ACPI_RSD_WORDLIST:
			...
			if (previous_target) {
				acpi_rs_dump_word_list (*previous_target,
					*(ACPI_CAST_INDIRECT_PTR (u16, target)));
			}
			break;
		...
		case ACPI_RSD_ADDRESS:
			...
			acpi_rs_dump_address_common (ACPI_CAST_PTR (
				union acpi_resource_data, target));
			break;
		...
		case ACPI_RSD_SOURCE:
			...
			acpi_rs_dump_resource_source (ACPI_CAST_PTR (
				struct acpi_resource_source, target));
			break;
		...
		case ACPI_RSD_LABEL:
			 ...
			 * resource_label
			 */
			acpi_rs_dump_resource_label ("Resource Label", ACPI_CAST_PTR (
				struct acpi_resource_label, target));
			break;
		...
		case ACPI_RSD_SOURCE_LABEL:
			 ...
			 */
			acpi_rs_dump_resource_label ("Resource Source Label", ACPI_CAST_PTR (
				struct acpi_resource_label, target));
			break;
