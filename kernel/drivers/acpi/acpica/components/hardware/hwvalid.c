	...
	u32                             bit_width)
{
	...
	ACPI_DEBUG_PRINT ((ACPI_DB_IO,
		...
		ACPI_FORMAT_UINT64 (address), ACPI_FORMAT_UINT64 (last_address),
		byte_width));
	...
	for (i = 0; i < ACPI_PORT_INFO_ENTRIES; i++, port_info++) {
		 ...
		 */
		if ((address <= port_info->end) && (last_address >= port_info->start)) {
			...
			if (acpi_gbl_osi_data >= port_info->osi_dependency) {
				ACPI_DEBUG_PRINT ((ACPI_DB_VALUES,
					...
					ACPI_FORMAT_UINT64 (address), byte_width, port_info->name,
