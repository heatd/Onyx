	...
	if ((arg->common.aml_opcode == AML_INT_NAMEPATH_OP) &&
		...
		else    /* All other opcodes */
		{
			...
			status = acpi_ns_lookup (walk_state->scope_info, name_string,
				...
				ACPI_NS_SEARCH_PARENT | ACPI_NS_DONT_OPEN_SCOPE, walk_state,
			...
			if (status == AE_NOT_FOUND) {
				if (parent_op->common.aml_opcode == AML_CONDITIONAL_REF_OF_OP) {
					/*
				...
				else if (parent_op->common.aml_opcode == AML_EXTERNAL_OP) {
					/*
	...
	struct acpi_walk_state          *walk_state)
{
	...
	if (type == ACPI_TYPE_INTEGER) {
		...
		status = acpi_ut_copy_iobject_to_iobject (
			*operand, &new_obj_desc, walk_state);
		if (ACPI_FAILURE (status)) {
			goto exit;
		}
	}
