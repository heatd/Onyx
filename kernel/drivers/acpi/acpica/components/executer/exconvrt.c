	...
	u32                             implicit_conversion)
{
	...
	case ACPI_TYPE_STRING:
		...
		if (implicit_conversion) {
			result = acpi_ut_implicit_strtoul64 (ACPI_CAST_PTR (char, pointer));
		}
		else {
			result = acpi_ut_explicit_strtoul64 (ACPI_CAST_PTR (char, pointer));
		}
		break;
	...
	union acpi_operand_object       **result_desc)
{
	...
	case ACPI_TYPE_INTEGER:
		...
		memcpy (new_buf, &obj_desc->integer.value, acpi_gbl_integer_byte_width);
		break;
	...
	u32                             type)
{
	...
	case ACPI_TYPE_INTEGER:
		...
		string_length = acpi_ex_convert_to_ascii (
			obj_desc->integer.value, base, new_buf, acpi_gbl_integer_byte_width);
	...
	case ACPI_TYPE_BUFFER:
		...
		for (i = 0; i < obj_desc->buffer.length; i++) {
			...
			new_buf += acpi_ex_convert_to_ascii (
				(u64) obj_desc->buffer.pointer[i], base, new_buf, 1);
