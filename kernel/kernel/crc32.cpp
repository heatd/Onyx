...
uint32_t crc32_calculate(const uint8_t *ptr, size_t len)
{
    ...
    while (len--)
        crc = crc32_table[(crc & 0xFF) ^ *(ptr++)] ^ (crc >> 8);
    return ~crc;
}
...
#ifdef __x86_64__
uint32_t crc32c_calculate_sse(const void *buf, size_t length, uint32_t initial_value);
#endif
...
{
#ifdef __x86_64__
    if (x86_has_cap(X86_FEATURE_SSE42))
        return crc32c_calculate_sse(buffer, length, initial_value);
#endif
    const uint8_t *Buf;
    uint32_t Crc;
...
uint16_t crc16_calculate(const void *Buffer, size_t Length, uint16_t InitialValue)
{
    ...
    while (Length-- != 0)
    {
        Crc = crc16_lookup_table[(Crc & 0xFF) ^ *(Buf++)] ^ (Crc >> 8);
    }
