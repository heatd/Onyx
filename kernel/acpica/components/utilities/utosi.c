	...
	acpi_string                     interface_name)
{
	...
	interface_info = ACPI_ALLOCATE_ZEROED (sizeof (struct acpi_interface_info));
	if (!interface_info) {
		return (AE_NO_MEMORY);
	}
		...
		if (!strcmp (interface_name, next_interface->name)) {
			...
			if (next_interface->flags & ACPI_OSI_DYNAMIC) {
				...
				if (previous_interface == next_interface) {
					...
					previous_interface->next = next_interface->next;
				}
	...
	struct acpi_walk_state          *walk_state)
{
	...
	if (!string_desc ||
		(string_desc->common.type != ACPI_TYPE_STRING)) {
