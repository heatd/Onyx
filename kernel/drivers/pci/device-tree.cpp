...
int enumerate_root_bus_resources(pci_root *root, device_tree::node *dev)
{
    ...
    if (!ranges)
    {
        pr_err("pci-dt: device has no ranges property\n");
        return -EIO;
    }
    ...
    while (ranges < end)
    {
        ...
        pr_info("pci-dt: range %s pci %lx cpu %lx size %lx\n", str, pci_address, cpu_address, size);
        ...
        if (flags != 0)
        {
            ...
            if (st < 0)
            {
                pr_error("pci-dt: error adding host bridge resource: %s\n", strerror(-st));
                return st;
            }
        }
    }
...
static int pci_root_probe(struct device *dev_)
{
    ...
    if (!ecam)
    {
        pr_err("error: pci-host-ecam-generic has no ecam range\n");
        return -EIO;
    }
    ...
    if (st < 0)
        return st;
    pr_info("pci-dt: Found ECAM@%016lx for buses [%u, %u]\n", ecam->start(), start_bus, end_bus);
