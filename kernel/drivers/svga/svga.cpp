...
{
    /* To set the video mode with SVGA, we need to write the width to _REG_WIDTH, height to
       _REG_HEIGHT, and bpp to _REG_BITS_PER_PIXEL
    */
...
static struct pci::pci_id svga_dev_ids[] = {
    {PCI_ID_DEVICE(VMWARE_SVGAII_PCI_VENDOR, VMWARE_SVGAII_PCI_DEVICE, NULL)}, {0}};

static struct driver svga_driver
{
    .name = "svga", .devids = &svga_dev_ids, .probe = svga_probe, .bus_type_node = {&svga_driver}
};
