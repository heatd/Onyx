...
int igpu_i2c_hw_rdy(struct igpu_device *dev)
{
    if (igpu_gpio_read(dev, GMBUS2) & GMBUS2_NAK_INDICATOR)
        return -1;
    return igpu_wait_bit(dev, dev->gpio_regs_off + GMBUS2, GMBUS2_HW_RDY, 50 * NS_PER_MS, false);
}

int i2c_wait_for_completion(struct igpu_device *dev)
{
    return igpu_wait_bit(dev, dev->gpio_regs_off + GMBUS2, GMBUS2_HW_WAIT_PHASE, 50 * NS_PER_MS,
                         false);
}
...
void igpu_i2c_finish_transaction(struct igpu_device *dev)
{
    ...
    int st =
        igpu_wait_bit(dev, dev->gpio_regs_off + GMBUS2, GMBUS2_GMBUS_ACTIVE, 100 * NS_PER_MS, true);
...
int igpu_transaction(struct i2c_adapter *adapter, struct i2c_message *msgs, size_t nr)
{
