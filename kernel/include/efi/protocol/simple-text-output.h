...
{
    EFI_STATUS(*Reset)
    (struct EFI_SIMPLE_TEXT_OUTPUT_PROTOCOL* self, bool extended_verification) EFIAPI;

    EFI_STATUS(*OutputString)
    (struct EFI_SIMPLE_TEXT_OUTPUT_PROTOCOL* self, char16_t* string) EFIAPI;

    EFI_STATUS (*TestString)(struct EFI_SIMPLE_TEXT_OUTPUT_PROTOCOL* self, char16_t* string) EFIAPI;

    EFI_STATUS(*QueryMode)
    (struct EFI_SIMPLE_TEXT_OUTPUT_PROTOCOL* self, size_t mode_num, size_t* cols,
     size_t* rows) EFIAPI;

    EFI_STATUS (*SetMode)(struct EFI_SIMPLE_TEXT_OUTPUT_PROTOCOL* self, size_t mode_num) EFIAPI;

    EFI_STATUS(*SetAttribute)
    (struct EFI_SIMPLE_TEXT_OUTPUT_PROTOCOL* self, size_t attribute) EFIAPI;

    EFI_STATUS (*ClearScreen)(struct EFI_SIMPLE_TEXT_OUTPUT_PROTOCOL* self) EFIAPI;

    EFI_STATUS(*SetCursorPosition)
    (struct EFI_SIMPLE_TEXT_OUTPUT_PROTOCOL* self, size_t col, size_t row) EFIAPI;
