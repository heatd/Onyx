	...
	void                            *region_context)
{
	...
	if (!mm || (address < mm->physical_address) ||
		...
		for (mm = mem_info->first_mm; mm; mm = mm->next_mm) {
			...
			if ((u64) address + length > (u64) mm->physical_address + mm->length) {
				continue;
			}
		...
		if (!logical_addr_ptr) {
			ACPI_ERROR ((AE_INFO,
				...
				ACPI_FORMAT_UINT64 (address), (u32) map_length));
			ACPI_FREE(mm);
			return_ACPI_STATUS (AE_NO_MEMORY);
		}
	...
	case ACPI_READ:
		...
		*value = 0;
		status = acpi_os_read_pci_configuration (
			pci_id, pci_register, value, bit_width);
		break;

	case ACPI_WRITE:

		status = acpi_os_write_pci_configuration (
			pci_id, pci_register, *value, bit_width);
		break;
	...
	void                            *region_context)
{
	...
	case ACPI_READ:
		...
		memcpy (ACPI_CAST_PTR (char, value), pointer, ACPI_DIV_8 (bit_width));
		break;

	case ACPI_WRITE:

		memcpy (pointer, ACPI_CAST_PTR (char, value), ACPI_DIV_8 (bit_width));
		break;
