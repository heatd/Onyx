...
#define trace_lock_acquire(...)
#define trace_lock_acquired(...)
#define trace_lock_contended(...)
...
static void __print_lock_name(struct held_lock *hlock, struct lock_class *class)
{
	...
	name = class->name;
	if (!name) {
		name = __get_key_name(class->key, str) ?: "<TODO>";
...
static char *thread_comm(struct task_struct *p)
{
	return p->owner ? p->owner->comm : "(kernel)";
}

static int task_pid_nr(struct task_struct *p)
{
	return p->owner ? p->owner->pid_ : 0;
}
...
static void lockdep_print_held_locks(struct task_struct *p)
{
	...
	if (!depth)
		printk("no locks held by %s/%d.\n", thread_comm(p), task_pid_nr(p));
	else
		printk("%d lock%s held by %s/%d:\n", depth,
		       str_plural(depth), thread_comm(p), task_pid_nr(p));
	/*
...
static bool assign_lock_key(struct lockdep_map *lock)
{
	 ...
	 */
	/* XXX spinlock does not have lock_class_key yet */
			...
			struct held_lock *check_tgt)
{
	...
	pr_warn("%s/%d is trying to acquire lock:\n",
		thread_comm(curr), task_pid_nr(curr));
			...
			struct lock_chain *chain)
{
	...
	print_kernel_ident();
	pr_warn("----------------------------\n");
	pr_warn("%s/%d: ", thread_comm(current), task_pid_nr(current));
		     ...
		     const void *mem_to, struct held_lock *hlock)
{
	...
	pr_warn("%s/%d is freeing memory %px-%px, with a lock still held there!\n",
		thread_comm(curr), task_pid_nr(curr), mem_from, mem_to-1);
	print_lock(hlock);
	lockdep_print_held_locks(curr);
...
static void print_held_locks_bug(void)
{
	...
	pr_warn("WARNING: %s/%d still has locks held!\n",
	       thread_comm(current), task_pid_nr(current));
	print_kernel_ident();
	pr_warn("------------------------------------\n");
	lockdep_print_held_locks(current);
	pr_warn("\nstack backtrace:\n");
...
/* HAX! */
void stack_trace_print(const unsigned long *entries, unsigned int num_entries, int spaces)
{
