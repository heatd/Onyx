	...
	u32                             *return_length)
{
	...
	if ((protocol_id > ACPI_MAX_PROTOCOL_ID) ||
		...
		ACPI_ERROR ((AE_INFO,
			"Invalid Field/AccessAs protocol ID: 0x%4.4X", protocol_id));
	...
	union acpi_operand_object       **ret_buffer_desc)
{
	...
	if (obj_desc->common.type == ACPI_TYPE_BUFFER_FIELD) {
		...
		(obj_desc->field.region_obj->region.space_id == ACPI_ADR_SPACE_SMBUS ||
		 obj_desc->field.region_obj->region.space_id == ACPI_ADR_SPACE_GSBUS ||
		 obj_desc->field.region_obj->region.space_id == ACPI_ADR_SPACE_IPMI ||
		 obj_desc->field.region_obj->region.space_id == ACPI_ADR_SPACE_PLATFORM_RT ||
		 obj_desc->field.region_obj->region.space_id == ACPI_ADR_SPACE_FIXED_HARDWARE)) {
	 ...
	 */
	buffer_length = (acpi_size) ACPI_ROUND_BITS_UP_TO_BYTES (
	...
	if ((obj_desc->common.type == ACPI_TYPE_LOCAL_REGION_FIELD) &&
		(obj_desc->field.region_obj->region.space_id == ACPI_ADR_SPACE_GPIO)) {
		...
		(obj_desc->field.region_obj->region.space_id == ACPI_ADR_SPACE_PLATFORM_COMM)) {
		/*
		...
		memcpy (buffer, obj_desc->field.region_obj->field.internal_pcc_buffer +
		obj_desc->field.base_byte_offset, (acpi_size) ACPI_ROUND_BITS_UP_TO_BYTES (
			obj_desc->field.bit_length));
	...
	union acpi_operand_object       **result_desc)
{
	...
	if (obj_desc->common.type == ACPI_TYPE_BUFFER_FIELD) {
		 ...
		 */
		data_length = (acpi_size) ACPI_ROUND_BITS_UP_TO_BYTES (
			obj_desc->field.bit_length);
		...
		if (MASTER_SUBSPACE_COMMAND (obj_desc->field.base_byte_offset)) {
			...
			status = acpi_ex_access_region (
				obj_desc, 0, (u64 *) obj_desc->field.region_obj->field.internal_pcc_buffer,
				ACPI_WRITE);
			return_ACPI_STATUS (status);
		}
		return (AE_OK);
