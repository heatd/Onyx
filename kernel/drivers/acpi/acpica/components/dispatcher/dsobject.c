...
create_new_object:
	...
	status = acpi_ds_init_object_from_op (
		walk_state, op, op->common.aml_opcode, &obj_desc);
	if (ACPI_FAILURE (status)) {
	...
	union acpi_parse_object         *op)
{
	...
	status = acpi_ds_build_internal_object (
		walk_state, op->common.value.arg, &obj_desc);
	if (ACPI_FAILURE (status)) {
		return_ACPI_STATUS (status);
	}
	...
	union acpi_operand_object       **ret_obj_desc)
{
	...
	case ACPI_TYPE_BUFFER:
		 ...
		 */
		obj_desc->buffer.node = ACPI_CAST_PTR (
	...
	case ACPI_TYPE_PACKAGE:
		 ...
		 * implementations.
		 */
		obj_desc->package.node = ACPI_CAST_PTR (
			struct acpi_namespace_node, walk_state->operands[0]);
	...
	case ACPI_TYPE_LOCAL_REFERENCE:
		...
		case AML_TYPE_LOCAL_VARIABLE:
			...
			obj_desc->reference.value = ((u32) opcode) - AML_FIRST_LOCAL_OP;
			obj_desc->reference.class = ACPI_REFCLASS_LOCAL;

			status = acpi_ds_method_data_get_node (ACPI_REFCLASS_LOCAL,
				obj_desc->reference.value, walk_state,
		...
		case AML_TYPE_METHOD_ARGUMENT:
			...
			obj_desc->reference.value = ((u32) opcode) - AML_FIRST_ARG_OP;
		...
		default: /* Object name or Debug object */
			...
			case AML_INT_NAMEPATH_OP:
				...
				if (op->common.node) {
					obj_desc->reference.object = op->common.node->object;
				}
				break;
