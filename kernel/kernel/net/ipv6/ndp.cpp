...
int ndp_handle_ns(netif *nif, packetbuf *buf)
{
	...
	return icmpv6::send_packet(data,
	   cul::slice<unsigned char>{(unsigned char *) &sol->nd_na_target, sizeof(buf_) - sizeof(icmp6_hdr)});
}
...
int ndp_submit_request(shared_ptr<neighbour>& ptr, const in6_addr& target_addr, struct netif *netif)
{
	...
	return icmpv6::send_packet(data,
	   cul::slice<unsigned char>{(unsigned char *) &sol->nd_ns_target, sizeof(buf_) - sizeof(icmp6_hdr)});
}
...
expected<shared_ptr<neighbour>, int> ndp_resolve(const in6_addr& ip, struct netif *netif)
{
	...
	if(ptr->flags & NEIGHBOUR_FLAG_UNINITIALISED)
	{
		 ...
		 */
		while(!(ptr->flags & NEIGHBOUR_FLAG_HAS_RESPONSE) && clocksource_get_time() - t0 <= ndp_response_timeout)
			sched_sleep_ms(15);
