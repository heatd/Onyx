	...
	struct acpi_resource            **resource_ptr)
{
	...
	status = acpi_rs_get_list_length (
		aml_buffer, aml_buffer_length, &list_size_needed);
	...
	status = acpi_ut_walk_aml_resources (NULL, aml_buffer, aml_buffer_length,
		acpi_rs_convert_aml_to_resources, &current_resource_ptr);
	if (status == AE_AML_NO_RESOURCE_END_TAG) {
		status = AE_OK;
	}
	if (ACPI_FAILURE (status)) {
	...
	struct acpi_buffer              *output_buffer)
{
	...
	status = acpi_ut_walk_aml_resources (NULL, aml_start, aml_buffer_length,
		acpi_rs_convert_aml_to_resources, &resource);
	if (ACPI_FAILURE (status)) {
		return_ACPI_STATUS (status);
	}
	...
	struct acpi_buffer              *output_buffer)
{
	...
	status = acpi_rs_get_pci_routing_table_length (
		package_object,&buffer_size_needed);
	if (ACPI_FAILURE (status)) {
		return_ACPI_STATUS (status);
	}
	...
	for (index = 0; index < number_of_elements; index++) {
			...
			case ACPI_TYPE_LOCAL_REFERENCE:
				...
				if (obj_desc->reference.class != ACPI_REFCLASS_NAME) {
					ACPI_ERROR ((AE_INFO,
						...
						index, obj_desc->reference.class));
					return_ACPI_STATUS (AE_BAD_DATA);
				}
				...
				status = acpi_ns_handle_to_pathname (
					(acpi_handle) node, &path_buffer, FALSE);
				if (ACPI_FAILURE (status)) {
					return_ACPI_STATUS (status);
				}
				...
				user_prt->length += (u32) strlen (user_prt->source) + 1;
				break;

			case ACPI_TYPE_STRING:

				strcpy (user_prt->source, obj_desc->string.pointer);
		...
		user_prt->length = (u32) ACPI_ROUND_UP_TO_64BIT (user_prt->length);
	...
	struct acpi_buffer              *output_buffer)
{
	...
	status = acpi_rs_get_aml_length (
		resource_list->pointer, resource_list->length, &aml_size_needed);
