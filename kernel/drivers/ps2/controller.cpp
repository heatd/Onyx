...
uint8_t ps2_send_command(struct ps2_controller *controller, uint8_t command, bool get_response,
                         uint8_t *response)
{
                                   ...
                                   uint8_t *response)
{
    ...
    if (get_response)
    {
         ...
         * from the data port
         */
        while (!(inb(port->controller->command_port) & PS2_STATUS_OUTPUT_BUFFER_FULL))
        {
            if (future <= clocksource_get_time())
            {
                return PS2_CMD_TIMEOUT;
...
uint8_t ps2_read_internal_ram(struct ps2_controller *controller, uint8_t index)
{
     ...
     * as it sounds...
     */
    ps2_send_command(controller, PS2_CMD_READ_INTERNAL_RAM + index, true, &internal_ram);
    return internal_ram;
}
...
void ps2_write_internal_ram(struct ps2_controller *controller, uint8_t index, uint8_t val)
{
    ps2_send_command(controller, PS2_CMD_WRITE_INTERNAL_RAM + index, false, NULL);
...
void ps2_write_controller_config(struct ps2_controller *controller, uint8_t val)
{
...
void ps2_disable_irqs(struct ps2_controller *controller)
{
    ...
    config &= ~(PS2_CTRLR_CONFIG_FIRST_PORT_IRQ | PS2_CTRLR_CONFIG_SECOND_PORT_IRQ |
...
int ps2_enable_irqs(struct ps2_controller *controller)
{
    ...
    byte |= PS2_CTRLR_CONFIG_FIRST_PORT_IRQ | PS2_CTRLR_CONFIG_SECOND_PORT_IRQ |
            PS2_CTRLR_CONFIG_FIRST_PORT_TRANSLATION;

    ps2_write_controller_config(controller, byte);

    for (unsigned int i = 0; i < controller->nr_ports; i++)
        if (install_irq(controller->ports[i].irq, ps2_irq, controller->device, 0,
                        &controller->ports[i]) < 0)
            return -1;
...
int ps2_reset_device(struct ps2_port *port)
{
    ...
    if (ps2_send_command_to_device(port, PS2_CMD_DEVICE_RESET, true, &response) == PS2_CMD_TIMEOUT)
    {
...
bool ps2_init_port(struct ps2_port *port)
{
    ...
    if (port->port_number == 1)
    {
        if (ps2_send_command(port->controller, PS2_CMD_TEST_FIRST_PORT, true, &response) ==
            PS2_CMD_TIMEOUT)
        {
    ...
    {
        if (ps2_send_command(port->controller, PS2_CMD_TEST_SECOND_PORT, true, &response) ==
            PS2_CMD_TIMEOUT)
        {
...
int ps2_probe(struct device *device)
{
    ...
    printf("ps2 controller cmd %02x; data %02x; kirq %u; mirq %u\n", i8042_command_port,
           i8042_data_port, i8042_keyboard_irq, i8042_mouse_irq);
...
static struct acpi_dev_id acpi_keyboard_ids[] = {
    {"PNP0300"}, {"PNP0301"}, {"PNP0302"}, {"PNP0303"}, {"PNP0304"}, {"PNP0305"},
    {"PNP0306"}, {"PNP0309"}, {"PNP030A"}, {"PNP030B"}, {"PNP0320"}, {"PNP0343"},
    {"PNP0344"}, {"PNP0345"}, {"CPQA0D7"}, {NULL},
};

static struct acpi_dev_id acpi_mouse_ids[] = {
    {"AUI0200"}, {"FCJ6000"}, {"FCJ6001"}, {"PNP0F03"}, {"PNP0F0B"}, {"PNP0F0E"},
    {"PNP0F12"}, {"PNP0F13"}, {"PNP0F19"}, {"PNP0F1C"}, {"SYN0801"}, {NULL},
};

int ps2_probe_keyboard(struct device *device);
int ps2_probe_mouse(struct device *device);

struct driver ps2_keyboard_driver = {.name = "ps2keyb",
                                     .devids = &acpi_keyboard_ids,
                                     .probe = ps2_probe_keyboard,
                                     .bus_type_node = {&ps2_keyboard_driver}};

struct driver ps2_mouse_driver = {.name = "ps2mouse",
