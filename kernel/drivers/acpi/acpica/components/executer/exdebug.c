	...
	u32                             index)
{
	...
	if (!((level > 0) && index == 0)) {
		if (acpi_gbl_display_debug_timer) {
			...
			acpi_os_printf ("ACPI Debug: T=0x%8.8X %*s", timer, level, " ");
		}
		else {
	...
	if (ACPI_GET_DESCRIPTOR_TYPE (source_desc) == ACPI_DESC_TYPE_OPERAND) {
		...
		if ((source_desc->common.type != ACPI_TYPE_INTEGER) &&
			(source_desc->common.type != ACPI_TYPE_STRING)) {
			acpi_os_printf ("%s ", acpi_ut_get_object_type_name (source_desc));
		}
	...
	case ACPI_TYPE_BUFFER:
		...
		acpi_ut_dump_buffer (source_desc->buffer.pointer,
			(source_desc->buffer.length < 256) ?
				source_desc->buffer.length : 256, DB_BYTE_DISPLAY, 0);
		break;
	...
	case ACPI_TYPE_LOCAL_REFERENCE:
		...
		acpi_os_printf ("[%s] ", acpi_ut_get_reference_name (source_desc));
		...
		if (source_desc->reference.node) {
			if (ACPI_GET_DESCRIPTOR_TYPE (source_desc->reference.node) !=
				ACPI_DESC_TYPE_NAMED) {
				acpi_os_printf (" %p - Not a valid namespace node\n",
					source_desc->reference.node);
			}
			else {
				acpi_os_printf ("Node %p [%4.4s] ", source_desc->reference.node,
					(source_desc->reference.node)->name.ascii);
			...
			if (ACPI_GET_DESCRIPTOR_TYPE (source_desc->reference.object) ==
				...
				acpi_ex_do_debug_object (ACPI_CAST_PTR (union acpi_operand_object,
				...
				case ACPI_TYPE_BUFFER:
					...
					acpi_os_printf ("Buffer[%u] = 0x%2.2X\n",
						value, *source_desc->reference.index_pointer);
					break;

				case ACPI_TYPE_STRING:

					acpi_os_printf ("String[%u] = \"%c\" (0x%2.2X)\n",
						value, *source_desc->reference.index_pointer,
						*source_desc->reference.index_pointer);
					break;
				...
				case ACPI_TYPE_PACKAGE:
					...
					if (!(*source_desc->reference.where)) {
						acpi_os_printf ("[Uninitialized Package Element]\n");
					}
					else {
						acpi_ex_do_debug_object (*source_desc->reference.where,
							level+4, 0);
					}
					break;
				...
				default:
					...
					acpi_os_printf ("Unknown Reference object type %X\n",
