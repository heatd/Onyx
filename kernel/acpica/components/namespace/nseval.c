	...
	struct acpi_evaluate_info       *info)
{
	...
	if (!info->node) {
		 ...
		 */
		status = acpi_ns_get_node (info->prefix_node, info->relative_pathname,
	...
	ACPI_DEBUG_PRINT_RAW ((ACPI_DB_EVALUATION,
		"%-26s:  %s (%s)\n", "   Enter evaluation",
		&info->full_pathname[1], acpi_ut_get_type_name (info->node->type)));
	...
	case ACPI_TYPE_LOCAL_SCOPE:
		...
		ACPI_ERROR ((AE_INFO,
			...
			info->full_pathname, acpi_ut_get_type_name (info->node->type)));
	...
	default:
		...
		status = acpi_ex_resolve_node_to_value (ACPI_CAST_INDIRECT_PTR (
			struct acpi_namespace_node, &info->return_object), NULL);
		acpi_ex_exit_interpreter ();
