	...
	u32                             length)
{
	...
	length = table->length;
	checksum = acpi_ut_generate_checksum (ACPI_CAST_PTR (u8, table), length, table->checksum);
	...
	u32                             length)
{
	...
	checksum = acpi_ut_generate_checksum (ACPI_CAST_PTR (u8, cdat_table),
			  cdat_table->length, cdat_table->checksum);
	...
	if (checksum != cdat_table->checksum) {
		ACPI_BIOS_WARNING ((AE_INFO,
			"Incorrect checksum in table [%4.4s] - 0x%2.2X, "
			"should be 0x%2.2X",
			acpi_gbl_CDAT, cdat_table->checksum, checksum));
