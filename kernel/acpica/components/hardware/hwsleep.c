	...
	u8                              sleep_state)
{
	...
	sleep_type_reg_info = acpi_hw_get_bit_register_info (ACPI_BITREG_SLEEP_TYPE);
	...
	u8                              sleep_state)
{
	...
	if (acpi_gbl_sleep_type_AS0 != ACPI_SLEEP_TYPE_INVALID) {
		...
		if (ACPI_SUCCESS (status)) {
			...
			(void) acpi_hw_write_pm1_control (pm1a_control, pm1b_control);
		}
	}
	...
	u8                              sleep_state)
{
	...
	if ((acpi_gbl_FADT.flags & ACPI_FADT_PCI_EXPRESS_WAKE)) {
		(void) acpi_write_bit_register (
		acpi_gbl_fixed_event_info[ACPI_EVENT_PCIE_WAKE].enable_register_id,
		ACPI_DISABLE_EVENT);
		(void) acpi_write_bit_register (
		acpi_gbl_fixed_event_info[ACPI_EVENT_PCIE_WAKE].status_register_id,
		ACPI_CLEAR_STATUS);
	}
