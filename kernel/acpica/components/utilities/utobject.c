	...
	acpi_object_type                type)
{
	...
	object = acpi_ut_allocate_object_desc_dbg (
		module_name, line_number, component_id);
	if (!object) {
		return_PTR (NULL);
	}
	...
	u32                             count)
{
	 ...
	 * terminated.
	 */
	package_elements = ACPI_ALLOCATE_ZEROED (
	...
	acpi_size                       *obj_length)
{
	...
	if (ACPI_GET_DESCRIPTOR_TYPE (internal_object) == ACPI_DESC_TYPE_NAMED) {
		...
		ACPI_ERROR ((AE_INFO,
			...
			ACPI_CAST_PTR (struct acpi_namespace_node, internal_object)->name.ascii));
	...
	case ACPI_TYPE_LOCAL_REFERENCE:
		...
		case ACPI_REFCLASS_NAME:
			 ...
			 */
			size = acpi_ns_get_pathname_length (internal_object->reference.node);
			if (!size) {
		...
		default:
			...
			ACPI_ERROR ((AE_INFO, "Cannot convert to external object - "
				...
				internal_object->reference.class, internal_object));
			status = AE_TYPE;
			break;
		}
		break;
	...
	acpi_size                       *obj_length)
{
	...
	status = acpi_ut_walk_package_tree (
		internal_object, NULL, acpi_ut_get_element_length, &info);
	if (ACPI_FAILURE (status)) {
		return_ACPI_STATUS (status);
	}
	...
	info.length += ACPI_ROUND_UP_TO_NATIVE_WORD (
		sizeof (union acpi_object)) * (acpi_size) info.num_packages;
