...
{
    // We need this lock because there might be a race between the woken task freeing up the
    // structure and wait_queue_wake_all doing the stuffs
    spinlock lock_;
...
struct virtio_allocation_info
{
    ...
    constexpr virtio_allocation_info()
        : vec{}, nr_vecs{}, alloc_flags{}, first_desc{}, fill_function{}, context{}, completion{}
    {
    }
};
...
public:
    virtq_split(vdev *dev, unsigned int qsize, unsigned int nr)
        : virtq{dev, nr}, vq_pages{nullptr}, queue_size{qsize}, descs{nullptr}, avail{nullptr},
          used{nullptr}, eff_queue_notify_off{0}, last_seen_used_idx{0}
    {
...
public:
    ...
    constexpr virtio_structure()
        : bar{nullptr}, is_io_port{false}, offset{0}, length{0}, initialized{false},
          notify_off_mult{~0U}
    {
    }
