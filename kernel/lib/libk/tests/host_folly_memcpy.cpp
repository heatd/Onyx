...
// Compile using:
// g++ -O2 kernel/lib/libk/tests/host_folly_memcpy.cpp kernel/lib/libk/arch/x86_64/memcpy.S
...
extern "C" void *__memmove(void *, const void *, size_t);
...
void testLen(size_t len, size_t dst_offset = 0, size_t src_offset = 0) {
  ...
  for (size_t i = 0; i < len; ++i) {
    ASSERT_EQ(src[i + src_offset], expected_src(i + src_offset))
        << "__memcpy(dst+" << dst_offset << ", src+" << src_offset << ", "
        << len << "), i = " << i;
    ...
    for (ssize_t overlap_offset = -copy_size - 1;
      ...
      for (ssize_t i = 0; i < copy_size; i++) {
        ASSERT_EQ(
            *(copy_buf.data() + kStartIndex + overlap_offset + i),
            *(check_buf.data() + i))
            << "Error after __memmove(src + " << kStartIndex << " + "
