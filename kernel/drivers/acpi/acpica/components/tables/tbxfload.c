	...
	struct acpi_table_header        *table)
{
	...
	status = acpi_tb_install_standard_table (ACPI_PTR_TO_PHYSADDR (table),
		ACPI_TABLE_ORIGIN_EXTERNAL_VIRTUAL, table, FALSE, FALSE, &table_index);
	...
	acpi_physical_address           address)
{
	...
	status = acpi_tb_install_standard_table (address,
		ACPI_TABLE_ORIGIN_INTERNAL_PHYSICAL, NULL, FALSE, FALSE, &table_index);
	...
	u32                             *table_idx)
{
	...
	status = acpi_tb_install_and_load_table (ACPI_PTR_TO_PHYSADDR (table),
		ACPI_TABLE_ORIGIN_EXTERNAL_VIRTUAL, table, FALSE, &table_index);
	if (table_idx) {
...
acpi_unload_parent_table (
	acpi_handle                     object)
{
	struct acpi_namespace_node      *node = ACPI_CAST_PTR (struct acpi_namespace_node, object);
