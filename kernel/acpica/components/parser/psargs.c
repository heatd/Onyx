	...
	u8                              possible_method_call)
{
	...
	if (ACPI_SUCCESS (status) &&
		...
		if ((GET_CURRENT_ARG_TYPE (walk_state->arg_types) == ARGP_SUPERNAME) ||
			(GET_CURRENT_ARG_TYPE (walk_state->arg_types) == ARGP_TARGET)) {
			/*
	...
	if (status == AE_NOT_FOUND) {
		...
		else if (walk_state->op->common.aml_opcode == AML_CONDITIONAL_REF_OF_OP) {
			status = AE_OK;
		}
		 ...
		 */
			...
			((arg->common.parent->common.aml_opcode == AML_PACKAGE_OP) ||
			 (arg->common.parent->common.aml_opcode == AML_VARIABLE_PACKAGE_OP))) {
			status = AE_OK;
		}
	}
	...
	struct acpi_parse_state         *parser_state)
{
	...
	case AML_INT_NAMEDFIELD_OP:
		...
		if (field->common.inline_comment) {
			field->common.name_comment = field->common.inline_comment;
		}
	...
	case AML_INT_EXTACCESSFIELD_OP:
		...
		if (opcode == AML_INT_EXTACCESSFIELD_OP) {
			...
			field->common.value.integer |= (u32) (access_length << 16);
		}
		break;
	...
	case AML_INT_CONNECTION_OP:
		...
		if (ACPI_GET8 (parser_state->aml) == AML_BUFFER_OP) {
			...
			pkg_length = acpi_ps_get_next_package_length (parser_state);
			pkg_end += pkg_length;
			...
			if (parser_state->aml < pkg_end) {
				...
				arg = acpi_ps_alloc_op (AML_INT_BYTELIST_OP, aml);
				if (!arg) {
				...
				case AML_BYTE_OP:       /* AML_BYTEDATA_ARG */
					...
					buffer_length = ACPI_GET8 (parser_state->aml);
					parser_state->aml += 1;
					break;

				case AML_WORD_OP:       /* AML_WORDDATA_ARG */

					buffer_length = ACPI_GET16 (parser_state->aml);
					parser_state->aml += 2;
					break;

				case AML_DWORD_OP:      /* AML_DWORDATA_ARG */

					buffer_length = ACPI_GET32 (parser_state->aml);
					parser_state->aml += 4;
					break;
	...
	union acpi_parse_object         **return_arg)
{
	...
	case ARGP_NAME_OR_REF:
		...
		if (subop == 0                  ||
			...
			arg = acpi_ps_alloc_op (AML_INT_NAMEPATH_OP, parser_state->aml);
			if (!arg) {
				return_ACPI_STATUS (AE_NO_MEMORY);
			}

			status = acpi_ps_get_next_namepath (walk_state, parser_state,
