	...
	union acpi_operand_object       **obj_desc_ptr)
{
	...
	if (!obj_desc->package.elements) {
		obj_desc->package.elements = ACPI_ALLOCATE_ZEROED (
			((acpi_size) element_count + 1) * sizeof (void *));
	...
	for (i = 0; arg && (i < element_count); i++) {
		if (arg->common.aml_opcode == AML_INT_RETURN_VALUE_OP) {
			...
			if (arg->common.node->type == ACPI_TYPE_METHOD) {
				...
				status = acpi_ds_build_internal_object (
					walk_state, arg, &obj_desc->package.elements[i]);
			}
			else {
			...
			if (!module_level_code) {
				 ...
				 */
				acpi_ds_init_package_element (0, obj_desc->package.elements[i],
					NULL, &obj_desc->package.elements[i]);
			}
		}
		...
		if (*obj_desc_ptr) {
			...
			if (reference_count > 1) {
				...
				for (index = 0; index < ((u32) reference_count - 1); index++) {
	...
	if (arg) {
		...
		ACPI_DEBUG_PRINT_RAW ((ACPI_DB_INFO,
			...
			ACPI_GET_FUNCTION_NAME, i, element_count));
	}
	...
	union acpi_operand_object       **element_ptr)
{
	...
	if (ACPI_FAILURE (status)) {
		if ((status == AE_NOT_FOUND) && acpi_gbl_ignore_package_resolution_errors) {
			/*
		...
		status2 = acpi_ns_externalize_name (ACPI_UINT32_MAX,
			(char *) element->reference.aml, NULL, &external_path);
		...
		*element_ptr = NULL;
		...
		ACPI_ERROR ((AE_INFO,
			...
			resolved_node->name.ascii, scope_info.scope.node->name.ascii));
		*element_ptr = NULL;
		return_VOID;
	}
