	...
	union acpi_parse_object         *op)
{
	...
	case AML_INT_NAMEPATH_OP:   /* AML_NAMESTRING_ARG */
		...
		status = acpi_ps_get_next_namepath (walk_state,
			&(walk_state->parser_state), op, ACPI_POSSIBLE_METHOD_CALL);
	...
	default:
		...
		case AML_METHOD_OP:
			...
			op->named.length = (u32)
				(walk_state->parser_state.pkg_end - walk_state->parser_state.aml);
	...
	struct acpi_walk_state          *walk_state)
{
	...
	while ((parser_state->aml < parser_state->aml_end) || (op)) {
		...
		if (!op) {
			status = acpi_ps_create_op (walk_state, aml_op_start, &op);
			if (ACPI_FAILURE (status)) {
				/*
				...
				if (acpi_ns_opens_scope (
					acpi_ps_get_opcode_info (walk_state->opcode)->object_type)) {
					/*
					 ...
					 */
					opcode_length = 1;
					if ((walk_state->opcode & 0xFF00) == AML_EXTENDED_OPCODE) {
						opcode_length = 2;
					}
					walk_state->parser_state.aml = walk_state->aml + opcode_length;

					walk_state->parser_state.aml =
						acpi_ps_get_next_package_end(&walk_state->parser_state);
					walk_state->aml = walk_state->parser_state.aml;
				}
		...
		if (walk_state->arg_types) {
			...
			if (ACPI_FAILURE (status)) {
				...
				if ((walk_state->control_state) &&
					 ...
					 */
					parser_state->aml =
						walk_state->control_state->control.aml_predicate_start + 1;
					parser_state->aml =
					...
					if (*walk_state->aml == AML_ELSE_OP) {
						...
						walk_state->parser_state.aml = walk_state->aml + 1;
						walk_state->parser_state.aml =
							acpi_ps_get_next_package_end (parser_state);
						walk_state->aml = parser_state->aml;
					}
					ACPI_FREE(acpi_ut_pop_generic_state (&walk_state->control_state));
				}
				op = NULL;
				continue;
			}
		...
		if (walk_state->ascending_callback != NULL) {
			...
			else if ((walk_state->parse_flags & ACPI_PARSE_MODULE_LEVEL) &&
