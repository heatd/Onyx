...
bool test_printf(const char* expected, const char* format, ...)
{
    ...
    if (buf[length] != '\0')
    {
        printk("missing string terminator\n");
        success = false;
    }
    if (length != (int) strlen(expected) || memcmp(buf, expected, length + 1) != 0)
    {
        printk("expected: \"%s\" (length %zu)\n", expected, strlen(expected));
        printk("but got:  \"%s\" (length %zu) with return value %d)\n", buf, strlen(buf), length);
        success = false;
    }
    return success;
}
...
TEST(printf, numbers)
{
    ...
    EXPECT_TRUE(test_printf("long:  -12345678 0 12345678", "long:  %ld %ld %ld", -12345678L, 0L,
                            12345678L));
    ...
    {
        EXPECT_TRUE(test_printf("ulong: 4282621618 0 12345678", "ulong: %lu %lu %lu", -12345678UL,
                                0UL, 12345678UL));
    }

    EXPECT_TRUE(test_printf("longlong: -12345678 0 12345678", "longlong: %lli %lli %lli",
    ...
    {
        EXPECT_TRUE(test_printf("ptrdiff_t (u): 4282621618 0 12345678",
                                "ptrdiff_t (u): %tu %tu %tu", (ptrdiff_t) -12345678, (ptrdiff_t) 0,
                                (ptrdiff_t) 12345678));
    }
}
...
TEST(printf, hex)
{
    ...
    if constexpr (kIs64Bit<unsigned long>)
    {
        EXPECT_TRUE(test_printf("ulong: ffffffffff439eb2 0 bc614e", "ulong: %lx %lx %lx",
                                -12345678UL, 0UL, 12345678UL));
    }
    else
    {
    ...
    {
        EXPECT_TRUE(test_printf("usize_t: ff439eb2 0 bc614e", "usize_t: %zx %zx %zx",
                                (size_t) -12345678, (size_t) 0, (size_t) 12345678));
    }
}

TEST(printf, alt_and_sign)
{
...
TEST(printf, field_width_precision)
{
    ...
    /* DOWNSTREAM */
    /* Make sure precision + field width behave with numbers too */
    /* 0 can be tricky, as it engages the precision logic too */
    EXPECT_TRUE(test_printf("00000", "%05d", 0));
    EXPECT_TRUE(test_printf("00001", "%05d", 1));
    EXPECT_TRUE(test_printf("10", "%.1d", 10));
    EXPECT_TRUE(test_printf("0000000010", "%.10d", 10));
    EXPECT_TRUE(test_printf("  010", "%5.3d", 10));
    EXPECT_TRUE(test_printf("100", "%02d", 100));
}

// Test snprintf() when the output is larger than the given buffer.
TEST(printf, snprintf_truncation)
{
    char buf[32];
