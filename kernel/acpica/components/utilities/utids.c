	...
	struct acpi_pnp_device_id       **return_id)
{
	...
	hid = ACPI_ALLOCATE_ZEROED (
		sizeof (struct acpi_pnp_device_id) + (acpi_size) length);
	if (!hid) {
		status = AE_NO_MEMORY;
		goto cleanup;
	}
	...
	hid->string = ACPI_ADD_PTR (char, hid, sizeof (struct acpi_pnp_device_id));
	...
	struct acpi_pnp_device_id       **return_id)
{
	...
	uid->string = ACPI_ADD_PTR (char, uid, sizeof (struct acpi_pnp_device_id));
	...
	for (i = 0; i < count; i++) {
		if (cid_objects[i]->common.type == ACPI_TYPE_INTEGER) {
			...
			acpi_ex_eisa_id_to_string (
				next_id_string, cid_objects[i]->integer.value);
			length = ACPI_EISAID_STRING_SIZE;
		}
	...
	struct acpi_pnp_device_id       **return_id)
{
	...
	if (obj_desc->common.type == ACPI_TYPE_PACKAGE) {
		if (count > 0 && cls_objects[0]->common.type == ACPI_TYPE_INTEGER) {
			class_code[0] = (u8) cls_objects[0]->integer.value;
		}
		if (count > 1 && cls_objects[1]->common.type == ACPI_TYPE_INTEGER) {
			class_code[1] = (u8) cls_objects[1]->integer.value;
		}
		if (count > 2 && cls_objects[2]->common.type == ACPI_TYPE_INTEGER) {
	...
	cls->string = ACPI_ADD_PTR (char, cls, sizeof (struct acpi_pnp_device_id));
