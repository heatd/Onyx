	...
	if (node->type != ACPI_TYPE_METHOD) {
		if (required_param_count > 0) {
			...
			ACPI_BIOS_ERROR_PREDEFINED ((AE_INFO, pathname, ACPI_WARN_ALWAYS,
				...
				acpi_ut_get_type_name (node->type), required_param_count));
		}
		else if (!required_param_count && !predefined->info.expected_btypes) {
		...
		if (node->type != ACPI_TYPE_METHOD) {
			if (user_param_count) {
				ACPI_INFO_PREDEFINED ((AE_INFO, pathname, ACPI_WARN_ALWAYS,
					...
					user_param_count, acpi_ut_get_type_name (node->type)));
			}
