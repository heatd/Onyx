	...
	void                            **return_value)
{
	...
	if (obj_desc->common.type == ACPI_TYPE_DEVICE) {
		...
		next_handler_obj = acpi_ev_find_region_handler (
			handler_obj->address_space.space_id, obj_desc->common_notify.handler);
		if (next_handler_obj) {
	...
	void                            *context)
{
	...
	status = acpi_ns_walk_namespace (ACPI_TYPE_ANY, node,
		...
		acpi_ev_install_handler, NULL, handler_obj, NULL);
