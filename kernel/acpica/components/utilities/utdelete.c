	...
	union acpi_operand_object       *object)
{
	...
	case ACPI_TYPE_MUTEX:
		...
		if (object == acpi_gbl_global_lock_mutex) {
			...
			(void) acpi_os_delete_semaphore (acpi_gbl_global_lock_semaphore);
	...
	case ACPI_TYPE_METHOD:
		...
		if (object->method.mutex) {
			acpi_os_delete_mutex (object->method.mutex->mutex.os_mutex);
	...
	case ACPI_TYPE_REGION:
		...
		if (second_desc) {
			...
			if (handler_desc) {
				next_desc = handler_desc->address_space.region_list;
				start_desc = next_desc;
				last_obj_ptr = &handler_desc->address_space.region_list;
				...
				if (handler_desc->address_space.handler_flags &
					...
					if (handler_desc->address_space.setup) {
						(void) handler_desc->address_space.setup (object,
	...
	ACPI_DEBUG_PRINT_RAW ((ACPI_DB_ALLOCATIONS, "%s: Deleting Object %p [%s]\n",
		ACPI_GET_FUNCTION_NAME, object, acpi_ut_get_object_type_name (object)));
	...
	u32                             action)
{
	...
	case REF_INCREMENT:
		...
		ACPI_DEBUG_PRINT ((ACPI_DB_ALLOCATIONS,
			...
			acpi_ut_get_object_type_name (object), new_count));
		message = "Incremement";
		break;
	...
	case REF_DECREMENT:
		...
		ACPI_DEBUG_PRINT_RAW ((ACPI_DB_ALLOCATIONS,
			...
			ACPI_GET_FUNCTION_NAME, object, object->common.type, new_count));
	...
	while (object) {
		...
		case ACPI_TYPE_THERMAL:
			...
			for (i = 0; i < ACPI_NUM_NOTIFY_TYPES; i++) {
				prev_object = object->common_notify.notify_list[i];
				while (prev_object) {
					next_object = prev_object->notify.next[i];
					acpi_ut_update_ref_count (prev_object, action);
		...
		case ACPI_TYPE_PACKAGE:
			...
			for (i = 0; i < object->package.count; i++) {
				...
				case ACPI_TYPE_BUFFER:
					 ...
					 */
					acpi_ut_update_ref_count (next_object, action);
					break;
	...
	union acpi_operand_object       *object)
{
	...
	ACPI_DEBUG_PRINT_RAW ((ACPI_DB_ALLOCATIONS,
		...
		ACPI_GET_FUNCTION_NAME, object, object->common.reference_count));
