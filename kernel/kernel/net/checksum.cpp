...
inetsum_t do_checksum(const uint8_t *buf, size_t length)
{
	 ...
	 * 32bit aligned, and finally 64bit aligned. Then we have a main loop that adds 64 bytes together,
	 * using 'adc'-like instructions(add the registers together + the carry bit).
	 * Then, we finally handle trailing portions of the buffer(by testing 32-bit chunks,
	 * and then 16-bit chunks, and then handling the trailing byte).
	 * This seems to be the standard of doing this in software.
	 */
