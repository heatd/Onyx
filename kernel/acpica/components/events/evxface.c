...
acpi_install_notify_handler (
	...
	void                            *context)
{
	struct acpi_namespace_node      *node = ACPI_CAST_PTR (struct acpi_namespace_node, device);
	...
	while (next_sci_handler) {
		if (next_sci_handler->address == address) {
			...
			if (prev_sci_handler) {
				...
				acpi_gbl_sci_handler_list = next_sci_handler->next;
			}
	...
	void                            *context)
{
	...
	if (ACPI_FAILURE (status)) {
		...
		ACPI_DEBUG_PRINT ((ACPI_DB_INFO,
			...
			acpi_ut_get_event_name (event), event, handler));
	}
	...
	void                            *context)
{
	...
	if (((ACPI_GPE_DISPATCH_TYPE (handler->original_flags) ==
		...
		if (type != (u32) (gpe_event_info->flags & ACPI_GPE_XRUPT_TYPE_MASK)) {
	...
	gpe_event_info->flags |= (u8) (type | (is_raw_handler ?
