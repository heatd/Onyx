...
int ddaux_do_transfers(struct i2c_adapter *adapter, struct i2c_message *messages, size_t nr);

#define LITTLE_TO_BIG32(n) \
...
int ddaux_do_transfer(struct i2c_adapter *adapter, struct i2c_message *message)
{
...
int igd_init_displayport(struct igpu_device *dev)
{
    for (unsigned int i = 0; i < NR_DISPLAY_PORTS; i++)
    {
        ...
        dev->dports[i]->data_base_reg = DDI_GET_REG(DDI_AUX_DATA_BASE, i);
