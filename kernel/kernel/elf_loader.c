...
#include <stdbool.h>
extern int errno = 0;
int elf_parse_program_header(Elf32_Phdr* prog_hdr,Elf32_Half entries,void* file)
{
	for(int i = 0; i <= entries; i++)
	{
		if(prog_hdr[i].p_type == 1)
		{
			if(vmm_mark_addr_as_used((void*)prog_hdr[i].p_vaddr,prog_hdr[i].p_memsz / 1024) == 1)
			{
				return 1;
			}
