...
public:
    ...
    auto do_mapping_shared() -> struct vm_region *
    {
        ...
        // printk("Ptr: %lx - %lx\n", (unsigned long) vm->base, (unsigned long) vm->base + (1024 <<
        // PAGE_SHIFT));
    ...
    void *do_mapping_regular()
    {
        ...
        // printk("Ptr: %lx - %lx\n", (unsigned long) ptr, (unsigned long) ptr + (1024 <<
        // PAGE_SHIFT));
        return ptr;
    }
...
bool vm_unmap_tests::is_not_present_in_vmo(struct vm_object *vmo, unsigned long lower,
...
bool vm_protect_tests::execute()
{
    ...
    vm_mprotect(&kernel_address_space, (void *) p, 10 << PAGE_SHIFT, VM_READ | VM_WRITE);
...
static test *tests[] = {&unmap_test, &protect_test};
...
extern "C" void execute_vm_tests()
{
