	...
	struct acpi_resource_address64      *out)
{
	...
	case ACPI_RESOURCE_TYPE_ADDRESS16:
		...
		address16 = ACPI_CAST_PTR (
			struct acpi_resource_address16, &resource->data);
	...
	case ACPI_RESOURCE_TYPE_ADDRESS32:
		...
		address32 = ACPI_CAST_PTR (
			struct acpi_resource_address32, &resource->data);
	...
	case ACPI_RESOURCE_TYPE_ADDRESS64:
		...
		memcpy (out, &resource->data, sizeof (struct acpi_resource_address64));
		break;
	...
	struct acpi_buffer              *ret_buffer)
{
	...
	status = acpi_walk_resources (
		device_handle, name, acpi_rs_match_vendor_resource, &info);
	if (ACPI_FAILURE (status)) {
		return (status);
	}
	...
	void                                *context)
{
	...
	resource_end = ACPI_ADD_PTR (
		struct acpi_resource, buffer->pointer, buffer->length);
