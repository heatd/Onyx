	...
	struct acpi_namespace_node      **return_node)
{
	...
	if (ACPI_LV_NAMES & acpi_dbg_level) {
		...
		if (scope_name) {
			ACPI_DEBUG_PRINT ((ACPI_DB_NAMES,
				...
				scope_name, parent_node, ACPI_CAST_PTR (char, &target_name),
	...
	while (node) {
		...
		if (node->name.integer == target_name) {
			...
			ACPI_DEBUG_PRINT ((ACPI_DB_NAMES,
				...
				node, acpi_ut_get_node_name (parent_node), parent_node));
	...
	ACPI_DEBUG_PRINT ((ACPI_DB_NAMES,
		...
		"%p first child %p\n",
		ACPI_CAST_PTR (char, &target_name), acpi_ut_get_type_name (type),
		acpi_ut_get_node_name (parent_node), parent_node, parent_node->child));
	...
	struct acpi_namespace_node      **return_node)
{
	...
	ACPI_DEBUG_PRINT ((ACPI_DB_NAMES,
		...
		acpi_ut_get_node_name (parent_node), ACPI_CAST_PTR (char, &target_name)));
	...
	while (parent_node) {
		...
		status = acpi_ns_search_one_scope (
			target_name, parent_node, ACPI_TYPE_ANY, return_node);
		...
		if (status == AE_OK) {
			...
			if (flags & ACPI_NS_OVERRIDE_IF_FOUND) {
				ACPI_DEBUG_PRINT ((ACPI_DB_NAMES,
					...
					ACPI_CAST_PTR(char, &target_name), interpreter_mode,
					(*return_node)->type, walk_state->owner_id));
