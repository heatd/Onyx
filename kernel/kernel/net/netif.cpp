                         ...
                         struct rtgenmsg *rth)
{
    ...
    {
        err = netif_dump_if(pbf, nif, nlsk->pid, nlh->nlmsg_seq);
        if (err)
            break;
    }
                         ...
                         struct rtgenmsg *rth)
{
    ...
    for (auto &nif : netif_list)
    {
        err = netif_dump_ifaddr(pbf, nif, nlsk->pid, nlh->nlmsg_seq, rth->rtgen_family);
        if (err)
            break;
    }
...
static int netif_getroute(struct netlink_sock *nlsk, struct packetbuf *pbf, struct nlmsghdr *nlh,
                          struct rtgenmsg *rth)
{
    int err;
    ...
    err = inet_getroute(nlsk, pbf, nlh, rth);
out:
    rtnl_unlock();
    return err || nl_done(pbf, nlsk->pid, nlh->nlmsg_seq, 0);
}
