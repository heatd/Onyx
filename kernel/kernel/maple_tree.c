...
/* Exported from linux commit d67978318827d06f1c0fa4c31343a279e9df6fde */
#include <onyx/maple_tree.h>
#include <onyx/types.h>
#include <onyx/xarray.h>
...
static struct slab_cache *maple_node_cache;
...
__always_inline enum maple_type mte_node_type(const struct maple_enode *entry)
{
    return ((unsigned long) entry >> MAPLE_NODE_TYPE_SHIFT) & MAPLE_NODE_TYPE_MASK;
}

__always_inline bool ma_is_dense(const enum maple_type type)
{
    return type < maple_leaf_64;
}

__always_inline bool ma_is_leaf(const enum maple_type type)
{
    return type < maple_range_64;
}

__always_inline bool mte_is_leaf(const struct maple_enode *entry)
{
 ...
 * below 4096
 */
__always_inline bool mt_is_reserved(const void *entry)
{
    return ((unsigned long) entry < MAPLE_RESERVED_RANGE) && xa_is_internal(entry);
}

__always_inline void mas_set_err(struct ma_state *mas, long err)
{
    mas->node = MA_ERROR(err);
    mas->status = ma_error;
}

__always_inline bool mas_is_ptr(const struct ma_state *mas)
{
    return mas->status == ma_root;
}

__always_inline bool mas_is_start(const struct ma_state *mas)
{
    return mas->status == ma_start;
}

__always_inline bool mas_is_none(const struct ma_state *mas)
{
    return mas->status == ma_none;
}

__always_inline bool mas_is_paused(const struct ma_state *mas)
{
    return mas->status == ma_pause;
}

__always_inline bool mas_is_overflow(struct ma_state *mas)
{
...
__always_inline struct maple_node *mte_to_node(const struct maple_enode *entry)
{
...
__always_inline bool ma_is_root(struct maple_node *node)
{
    return ((unsigned long) node->parent & MA_ROOT_PARENT);
}

__always_inline bool mte_is_root(const struct maple_enode *node)
{
...
__always_inline bool mt_is_alloc(struct maple_tree *mt)
{
 ...
 * parent type.
 */
static inline void mas_set_parent(struct ma_state *mas, struct maple_enode *enode,
 ...
 */
__always_inline unsigned int mte_parent_slot(const struct maple_enode *enode)
{
 ...
 */
__always_inline struct maple_node *mte_parent(const struct maple_enode *enode)
{
    return (void *) ((unsigned long) (mte_to_node(enode)->parent) & ~MAPLE_NODE_MASK);
}
 ...
 */
__always_inline bool ma_dead_node(const struct maple_node *node)
{
 ...
 * Return: true if dead, false otherwise.
 */
__always_inline bool mte_dead_node(const struct maple_enode *enode)
{
 ...
 * otherwise.
 */
__always_inline unsigned long mas_safe_pivot(const struct ma_state *mas, unsigned long *pivots,
 ...
 */
static inline unsigned long mas_safe_min(struct ma_state *mas, unsigned long *pivots,
...
__always_inline bool mt_locked(const struct maple_tree *mt)
{
...
__always_inline void *mt_slot_locked(struct maple_tree *mt, void __rcu **slots,
                                     unsigned char offset)
{
 ...
 */
__always_inline void *mas_slot_locked(struct ma_state *mas, void __rcu **slots,
                                      unsigned char offset)
{
 ...
 * Return: The entry stored in @slots at the @offset
 */
__always_inline void *mas_slot(struct ma_state *mas, void __rcu **slots, unsigned char offset)
{
 ...
 */
__always_inline void *mas_root(struct ma_state *mas)
{
 ...
 */
static inline void ma_set_meta(struct maple_node *mn, enum maple_type mt, unsigned char offset,
                               unsigned char end)
{
...
static void mas_node_count_gfp(struct ma_state *mas, int count, gfp_t gfp)
{
    ...
    if (allocated < (unsigned long) count)
    {
 ...
 */
__always_inline unsigned char ma_data_end(struct maple_node *node, enum maple_type type,
 ...
 */
static inline unsigned long ma_max_gap(struct maple_node *node, unsigned long *gaps,
                                       enum maple_type mt, unsigned char *off)
{
 ...
 */
static inline int mab_no_null_split(struct maple_big_node *b_node, unsigned char split,
                                    unsigned char slot_count)
{
 ...
 */
static inline int mab_calc_split(struct ma_state *mas, struct maple_big_node *bn,
                                 unsigned char *mid_split, unsigned long min)
{
    ...
    else
    {
        ...
        *mid_split = 0;
         ...
         */
        while ((split < slot_count - 1) && ((bn->pivot[split] - min) < slot_count - 1) &&
 ...
 */
static inline void mas_mab_cp(struct ma_state *mas, unsigned char mas_start, unsigned char mas_end,
                              struct maple_big_node *b_node, unsigned char mab_start)
{
    enum maple_type mt;
    struct maple_node *node;
    void __rcu **slots;
 ...
 */
static inline void mab_mas_cp(struct maple_big_node *b_node, unsigned char mab_start,
                              unsigned char mab_end, struct ma_state *mas, bool new_max)
{
    int i, j = 0;
 ...
 */
static inline struct maple_enode *mas_new_ma_node(struct ma_state *mas,
                                                  struct maple_big_node *b_node)
{
 ...
 */
static inline unsigned char mas_mab_to_node(struct ma_state *mas, struct maple_big_node *b_node,
 ...
 */
static inline void mas_set_split_parent(struct ma_state *mas, struct maple_enode *left,
                                        struct maple_enode *right, unsigned char *slot,
                                        unsigned char split)
{
    if (mas_is_none(mas))
        return;
 ...
 */
static inline void mte_mid_split_check(struct maple_enode **l, struct maple_enode **r,
                                       struct maple_enode *right, unsigned char slot,
 ...
 */
static inline void mast_cp_to_nodes(struct maple_subtree_state *mast, struct maple_enode *left,
...
static inline void mast_combine_cp_right(struct maple_subtree_state *mast)
{
    ...
    mas_mab_cp(mast->orig_r, mast->orig_r->offset + 1, mt_slot_count(mast->orig_r->node), mast->bn,
               mast->bn->b_end);
    mast->orig_r->last = mast->orig_r->max;
}
 ...
 */
static int mas_spanning_rebalance(struct ma_state *mas, struct maple_subtree_state *mast,
                                  unsigned char count)
{
    ...
    while (count--)
    {
        ...
        split =
            mas_mab_to_node(mas, mast->bn, &left, &right, &middle, &mid_split, mast->orig_l->min);
 ...
 */
static inline void mas_split_final_node(struct maple_subtree_state *mast, struct ma_state *mas,
                                        int height)
{
 ...
 */
static inline void mast_fill_bnode(struct maple_subtree_state *mast, struct ma_state *mas,
                                   unsigned char skip)
{
    bool cp = true;
    unsigned char split;
 ...
 */
static inline void mast_split_data(struct maple_subtree_state *mast, struct ma_state *mas,
                                   unsigned char split)
{
 ...
 */
static inline bool mas_push_data(struct ma_state *mas, int height, struct maple_subtree_state *mast,
                                 bool left)
{
 ...
 */
static inline bool mas_reuse_node(struct ma_wr_state *wr_mas, struct maple_big_node *bn,
                                  unsigned char end)
{
    /* Need to be rcu safe. */
                                                ...
                                                struct maple_big_node *b_node, unsigned char end)
{
    ...
    if ((b_end < mt_min_slots[b_type]) && (!mte_is_root(old_enode)) &&
                                            ...
                                            struct ma_wr_state *r_wr_mas)
{
    ...
    {
        r_mas->last = mas_safe_pivot(r_mas, r_wr_mas->pivots, r_wr_mas->type, r_mas->offset + 1);
        r_mas->offset++;
    }
}
...
int mas_alloc_cyclic(struct ma_state *mas, unsigned long *startp, void *entry,
                     unsigned long range_lo, unsigned long range_hi, unsigned long *next, gfp_t gfp)
{
...
__always_inline void mas_rewalk(struct ma_state *mas, unsigned long index)
{
retry:
    mas_set(mas, index);
    mas_state_walk(mas);
...
__always_inline bool mas_rewalk_if_dead(struct ma_state *mas, struct maple_node *node,
 ...
 *
 */
static bool mas_rev_awalk(struct ma_state *mas, unsigned long size, unsigned long *gap_min,
 ...
 */
static inline int mas_sparse_area(struct ma_state *mas, unsigned long min, unsigned long max,
 ...
 * @size: The size needed
 */
int mas_empty_area_rev(struct ma_state *mas, unsigned long min, unsigned long max,
                       unsigned long size)
{
 ...
 */
static inline unsigned char mte_dead_leaves(struct maple_enode *enode, struct maple_tree *mt,
                                            void __rcu **slots)
{
    struct maple_node *node;
    enum maple_type type;
...
static void __rcu **mte_dead_walk(struct maple_enode **enode, unsigned char offset)
{
    ...
    do
    {
        *enode = ma_enode_ptr(next);
        ...
        next = rcu_dereference_protected(slots[offset], lock_is_held(&rcu_callback_map));
        offset = 0;
...
static void mt_free_walk(struct rcu_head *head)
{
    ...
    do
    {
        ...
        if ((offset < mt_slots[type]) &&
            rcu_dereference_protected(slots[offset], lock_is_held(&rcu_callback_map)))
...
static inline void __rcu **mte_destroy_descend(struct maple_enode **enode, struct maple_tree *mt,
 ...
 */
__always_inline bool mas_find_setup(struct ma_state *mas, unsigned long max, void **entry)
{
    switch (mas->status)
    {
        case ma_active:
...
{
    maple_node_cache = kmem_cache_create("maple_node", sizeof(struct maple_node),
                                         sizeof(struct maple_node), SLAB_PANIC, NULL);
}
...
int mtree_alloc_range(struct maple_tree *mt, unsigned long *startp, void *entry, unsigned long size,
                      unsigned long min, unsigned long max, gfp_t gfp)
{
    int ret = 0;
 ...
 * free entries.
 */
int mtree_alloc_cyclic(struct maple_tree *mt, unsigned long *startp, void *entry,
                       unsigned long range_lo, unsigned long range_hi, unsigned long *next,
                       gfp_t gfp)
{
    int ret;
...
int mtree_alloc_rrange(struct maple_tree *mt, unsigned long *startp, void *entry,
                       unsigned long size, unsigned long min, unsigned long max, gfp_t gfp)
{
    int ret = 0;
...
#define spin_lock_nested(s, c) spin_lock(((void) (c), s))
/**
...
/* Tree validations */
static void mt_dump_node(const struct maple_tree *mt, void *entry, unsigned long min,
                         unsigned long max, unsigned int depth, enum mt_dump_format format);
static void mt_dump_range(unsigned long min, unsigned long max, unsigned int depth,
...
static void mt_dump_entry(void *entry, unsigned long min, unsigned long max, unsigned int depth,
                          enum mt_dump_format format)
{
...
static void mt_dump_range64(const struct maple_tree *mt, void *entry, unsigned long min,
                            unsigned long max, unsigned int depth, enum mt_dump_format format)
{
...
static void mt_dump_arange64(const struct maple_tree *mt, void *entry, unsigned long min,
                             unsigned long max, unsigned int depth, enum mt_dump_format format)
{
    ...
    for (i = 0; i < mt_slots[type]; i++)
    {
        ...
        if (mte_parent_slot(child) != i)
        {
            pr_err("Slot error at %p[%u]: child %p has pslot %u\n", mas_mn(mas), i,
                   mte_to_node(child), mte_parent_slot(child));
            MT_BUG_ON(mas->tree, 1);
        }
...
void mas_dump(const struct ma_state *mas)
{
    ...
    pr_err("     min=%lx max=%lx alloc=%p, depth=%u, flags=%x\n", mas->min, mas->max, mas->alloc,
           mas->depth, mas->mas_flags);
    if (mas->index > mas->last)
...
void mas_wr_dump(const struct ma_wr_state *wr_mas)
{
    ...
    pr_err("        type=%u off_end=%u, node_end=%u, end_piv=%lx\n", wr_mas->type,
           wr_mas->offset_end, wr_mas->mas->end, wr_mas->end_piv);
}
