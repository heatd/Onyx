...
public:
	...
	explicit neighbour(int _domain, const neigh_proto_addr& addr) :
	          hwaddr_{}, hwaddr_len_{}, domain{_domain},
	          flags{0}
	{
		if(_domain == AF_INET)
	...
	void set_validity(unsigned long validity)
	{
		if(validity != NEIGHBOUR_VALIDITY_STATIC)
		{
			...
			/* TODO: This API is clunky and we could and should have something like what userspace has. */
...
public:
	 ...
	 */
	cul::pair<shared_ptr<neighbour>, bool> add(const neigh_proto_addr& addr, bool only_lookup = false);
