	...
	acpi_size                       *size_needed)
{
	...
	resource_end = ACPI_ADD_PTR (struct acpi_resource, resource, resource_list_size);
	while (resource < resource_end) {
		...
		case ACPI_RESOURCE_TYPE_VENDOR:
			...
			if (resource->data.vendor.byte_length > 7) {
				...
				total_size = sizeof (struct aml_resource_large_header);
			}
		...
		case ACPI_RESOURCE_TYPE_EXTENDED_IRQ:
			 ...
			 */
			total_size = (acpi_rs_length) (total_size +
				((resource->data.extended_irq.interrupt_count - 1) * 4) +
		...
		case ACPI_RESOURCE_TYPE_GPIO:
			...
			total_size = (acpi_rs_length) (total_size +
				...
				resource->data.gpio.resource_source.string_length +
				resource->data.gpio.vendor_length);
		...
		case ACPI_RESOURCE_TYPE_PIN_FUNCTION:
			...
			total_size = (acpi_rs_length) (total_size +
				(resource->data.pin_function.pin_table_length * 2) +
				resource->data.pin_function.resource_source.string_length +
				resource->data.pin_function.vendor_length);
		...
		case ACPI_RESOURCE_TYPE_SERIAL_BUS:
			...
			total_size = acpi_gbl_aml_resource_serial_bus_sizes [
				resource->data.common_serial_bus.type];

			total_size = (acpi_rs_length) (total_size +
				resource->data.i2c_serial_bus.resource_source.string_length +
				resource->data.i2c_serial_bus.vendor_length);
		...
		case ACPI_RESOURCE_TYPE_PIN_CONFIG:
			...
			total_size = (acpi_rs_length) (total_size +
				(resource->data.pin_config.pin_table_length * 2) +
				resource->data.pin_config.resource_source.string_length +
				resource->data.pin_config.vendor_length);
		...
		case ACPI_RESOURCE_TYPE_PIN_GROUP:
			...
			total_size = (acpi_rs_length) (total_size +
				(resource->data.pin_group.pin_table_length * 2) +
				resource->data.pin_group.resource_label.string_length +
				resource->data.pin_group.vendor_length);
		...
		case ACPI_RESOURCE_TYPE_PIN_GROUP_FUNCTION:
			...
			total_size = (acpi_rs_length) (total_size +
				resource->data.pin_group_function.resource_source.string_length +
				resource->data.pin_group_function.resource_source_label.string_length +
				resource->data.pin_group_function.vendor_length);
		...
		case ACPI_RESOURCE_TYPE_PIN_GROUP_CONFIG:
			...
			total_size = (acpi_rs_length) (total_size +
				resource->data.pin_group_config.resource_source.string_length +
				resource->data.pin_group_config.resource_source_label.string_length +
				resource->data.pin_group_config.vendor_length);
		...
		resource = ACPI_ADD_PTR (struct acpi_resource, resource, resource->length);
	}
	...
	acpi_size                       *size_needed)
{
	...
	*size_needed = ACPI_RS_SIZE_MIN;        /* Minimum size is one end_tag */
	...
	while (aml_buffer < end_aml) {
		...
		status = acpi_ut_validate_resource (NULL, aml_buffer, &resource_index);
		...
		minimum_aml_resource_length = acpi_gbl_resource_aml_sizes[resource_index];
		...
		buffer = aml_buffer + acpi_ut_get_resource_header_length (aml_buffer);
		...
		case ACPI_RESOURCE_NAME_ADDRESS64:
			 ...
			 */
			extra_struct_bytes = acpi_rs_stream_option_length (
		...
		case ACPI_RESOURCE_NAME_EXTENDED_IRQ:
			...
			extra_struct_bytes += acpi_rs_stream_option_length (
				resource_length - extra_struct_bytes, minimum_aml_resource_length);
			break;
		...
		case ACPI_RESOURCE_NAME_SERIAL_BUS:
			...
			minimum_aml_resource_length = acpi_gbl_resource_aml_serial_bus_sizes[
		...
		if (acpi_ut_get_resource_type (aml_buffer) ==
			...
			buffer_size = acpi_gbl_resource_struct_serial_bus_sizes[
				aml_resource->common_serial_bus.type] + extra_struct_bytes;
		}
		else {
		...
		*size_needed += buffer_size;
		...
		ACPI_DEBUG_PRINT ((ACPI_DB_RESOURCES,
			...
			acpi_ut_get_descriptor_length (aml_buffer), ACPI_FORMAT_UINT64(*size_needed)));
	...
	acpi_size                       *buffer_size_needed)
{
	...
	for (index = 0; index < number_of_elements; index++) {
		...
		for (table_index = 0;
			 table_index < package_element->package.count && !name_found;
			 table_index++) {
