	...
	u32                             *table_index)
{
	...
	acpi_tb_init_table_descriptor (&acpi_gbl_root_table_list.tables[i],
		new_table_desc->address, new_table_desc->flags, new_table_desc->pointer);
	...
	u32                             *table_index)
{
	...
	status = acpi_tb_acquire_temp_table (&new_table_desc, address, flags, table);
	if (ACPI_FAILURE (status)) {
		ACPI_ERROR ((AE_INFO,
	...
	if (!reload &&
		...
		ACPI_INFO ((
			...
			new_table_desc.signature.ascii, ACPI_FORMAT_UINT64 (address)));
	...
	acpi_tb_install_table_with_override (&new_table_desc, override, table_index);
...
acpi_tb_override_table (
	struct acpi_table_desc          *old_table_desc)
{
	acpi_status                     status;
	struct acpi_table_desc          new_table_desc;
	struct acpi_table_header        *table;
	...
	if (ACPI_SUCCESS (status) && table) {
		acpi_tb_acquire_temp_table (&new_table_desc, ACPI_PTR_TO_PHYSADDR (table),
