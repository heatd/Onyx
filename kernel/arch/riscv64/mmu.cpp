...
static unsigned long vm_prots_to_mmu(unsigned int prots)
{
    auto flags = (prots & VM_NOEXEC ? 0 : RISCV_MMU_EXECUTE) |
                 (prots & VM_WRITE ? RISCV_MMU_WRITE : 0) |
                 (prots & VM_READ ? RISCV_MMU_READ : 0) |
                 (prots & VM_USER ? RISCV_MMU_USER : RISCV_MMU_GLOBAL) |
                 RISCV_MMU_VALID;
...
bool riscv_get_pt_entry(void *addr, uint64_t **entry_ptr, bool may_create_path, struct mm_address_space *mm);
...
void paging_init(void)
{
    ...
    boot_pt->entries[indices[riscv_paging_levels - 1] + 1] = riscv_pt_page_mapping(VERYHUGE512GB_SIZE) | flags;
}
...
static void mmu_acct_page_table(PML *pt, page_table_levels level, mmu_acct &acct)
{
    ...
    for (const auto pte : pt->entries)
    {
        ...
        if (level != PT_LEVEL)
        {
            mmu_acct_page_table((PML *) PHYS_TO_VIRT(PML_EXTRACT_ADDRESS(pte)), (page_table_levels) (level - 1), acct);
        }
        else
        {
...
void mmu_verify_address_space_accounting(mm_address_space *as)
{
    mmu_acct acct{};
    acct.as = as;
    mmu_acct_page_table((PML *) PHYS_TO_VIRT(as->arch_mmu.top_pt),
                        PML4_LEVEL,
                        acct);
