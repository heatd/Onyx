	...
	struct acpi_rsconvert_info      *info)
{
	...
	while (count) {
		...
		destination = ACPI_ADD_PTR (void, resource, info->resource_offset);
		...
		case ACPI_RSC_COUNT_GPIO_VEN:
			...
			resource->length = resource->length + (info->value * item_count);
			break;
		...
		case ACPI_RSC_MOVE64:
			...
			acpi_rs_move_data (destination, source, item_count, info->opcode);
			break;
		...
		case ACPI_RSC_MOVE_GPIO_PIN:
			...
			target = (char *) ACPI_ADD_PTR (void, resource,
				(resource->length - item_count * 2));
			*(u16 **) destination = ACPI_CAST_PTR (u16, target);

			/* Copy the PIN data */

			source = ACPI_ADD_PTR (void, aml, ACPI_GET16 (source));
			acpi_rs_move_data (target, source, item_count, info->opcode);
			break;
		...
		case ACPI_RSC_SOURCE:
			 ...
			 */
			resource->length +=
				acpi_rs_get_resource_source (aml_resource_length, info->value,
		...
		case ACPI_RSC_SOURCEX:
			...
			resource->length +=
				acpi_rs_get_resource_source (aml_resource_length, (acpi_rs_length)
					(((item_count - 1) * sizeof (u32)) + info->value),
		...
		case ACPI_RSC_BITMASK:
			 ...
			 */
			item_count = acpi_rs_decode_bitmask (ACPI_GET8 (source), destination);
			if (item_count) {
		...
		case ACPI_RSC_BITMASK16:
			...
			item_count = acpi_rs_decode_bitmask (temp16, destination);
			if (item_count) {
				resource->length += (item_count - 1);
			}
	...
	while (count) {
		...
		case ACPI_RSC_INITSET:
			...
			acpi_rs_set_resource_header (
				INIT_RESOURCE_TYPE (info), aml_length, aml);
			break;
		...
		case ACPI_RSC_COUNT_GPIO_VEN:
			...
			aml_length = (u16) (
				aml_length + (info->value * item_count));
			acpi_rs_set_resource_length (aml_length, aml);
			break;
		...
		case ACPI_RSC_SOURCEX:
			 ...
			 * Optional resource_source (Index and String)
			 */
			aml_length = acpi_rs_set_resource_source (
				aml, (acpi_rs_length) aml_length, source);
			acpi_rs_set_resource_length (aml_length, aml);
			break;
		...
		case ACPI_RSC_SOURCE:
			 ...
			 * complicated case used by the Interrupt() macro
			 */
			aml_length = acpi_rs_set_resource_source (aml, info->value, source);
			acpi_rs_set_resource_length (aml_length, aml);
			break;
		...
		case ACPI_RSC_BITMASK:
			...
			ACPI_SET8 (destination,
				acpi_rs_encode_bitmask (source,
					*ACPI_ADD_PTR (u8, resource, info->value)));
			break;
		...
		case ACPI_RSC_BITMASK16:
			 ...
			 * 16-bit encoded bitmask (IRQ macro)
			 */
			temp16 = acpi_rs_encode_bitmask (
				source, *ACPI_ADD_PTR (u8, resource, info->value));
