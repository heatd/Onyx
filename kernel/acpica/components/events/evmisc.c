	...
	u32                             notify_value)
{
	...
	if (obj_desc) {
		...
		handler_list_head = obj_desc->common_notify.notify_list[handler_list_id];
	}
	 ...
	 */
	if (!acpi_gbl_global_notify[handler_list_id].handler && !handler_list_head) {
	...
	ACPI_DEBUG_PRINT ((ACPI_DB_INFO,
		...
		notify_value, acpi_ut_get_notify_name (notify_value, ACPI_TYPE_ANY), node));
	...
	void                            *context)
{
	...
	while (handler_obj) {
		...
		handler_obj = handler_obj->notify.next[info->notify.handler_list_id];
	}
	...
	void)
{
	...
	if (acpi_gbl_events_initialized) {
		...
		for (i = 0; i < ACPI_NUM_FIXED_EVENTS; i++) {
			...
			if (ACPI_FAILURE (status)) {
				ACPI_ERROR ((AE_INFO,
					"Could not disable fixed event %u", (u32) i));
			}
		}
