	...
	u8                              upper)
{
	...
	if (number == 0) {
		*(pos++) = '0';
		...
		while (number) {
			(void) acpi_ut_divide (number, base, &number, &digit_index);
	...
	u8                              type)
{
	...
	need_prefix = ((type & ACPI_FORMAT_PREFIX) && base != 10) ? TRUE : FALSE;
	...
	if (need_prefix) {
		...
		if (base == 16) {
			string = acpi_ut_bound_string_output (
