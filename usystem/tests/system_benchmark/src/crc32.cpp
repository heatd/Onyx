...
static const uint32_t crc32c_lookup_table[256] = {
...
static void lookup_crc32_bench(benchmark::State &state)
{
    char *src = new char[state.range()];
    for (auto _ : state)
    {
        auto val = crc32c_calculate(src, state.range(), 0);
...
BENCHMARK(lookup_crc32_bench)->AT_COMMON_SIZES;
...
#define __crc32b(crc, val) __asm__ __volatile__("crc32b %1, %k0" : "+r"(crc) : "rm"(val))
#define __crc32w(crc, val) __asm__ __volatile__("crc32w %1, %k0" : "+r"(crc) : "rm"(val))
#define __crc32l(crc, val) __asm__ __volatile__("crc32l %1, %k0" : "+r"(crc) : "rm"(val))
#define __crc32q(crc, val) __asm__ __volatile__("crc32q %1, %q0" : "+r"(crc) : "rm"(val))
...
static void sse_crc32_bench(benchmark::State &state)
{
    char *src = new char[state.range()];
    for (auto _ : state)
    {
        auto val = crc32c_calculate_sse(src, state.range(), 0);
        benchmark::DoNotOptimize(val);
        benchmark::ClobberMemory();
    }
...
BENCHMARK(sse_crc32_bench)->AT_COMMON_SIZES;
