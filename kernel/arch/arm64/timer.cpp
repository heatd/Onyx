...
void arm64_timer_set_oneshot(hrtime_t in_future)
{
    ...
    auto ticks = arm64_get_time();
    if (now > in_future)
        in_future = now + 100;

    const auto future_timestamp = ticks + u64_mul_u64_fp32_64(in_future - now, ns_per_tick);
...
static irqstatus_t arm64_irq_timer(struct irq_context *ctx, void *cookie)
{
...
static struct driver driver = {
    .name = "arm64_generic_timer",
    .devids = nullptr,
    .probe = nullptr,
    .bus_type_node = {&driver},
};
