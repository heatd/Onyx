...
Window::Window(size_t window_id, unsigned int width, unsigned int height, unsigned int x,
               unsigned int y, std::weak_ptr<Display> display)
    : window_id(window_id), width(width), height(height), x(x), y(y), display(display), dirty(false)
{
