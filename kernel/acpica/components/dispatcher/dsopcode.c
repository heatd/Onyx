	...
	union acpi_operand_object       *result_desc)
{
	...
	if ((bit_offset + bit_count) >
		...
		ACPI_BIOS_EXCEPTION ((AE_INFO, status,
			...
			acpi_ut_get_node_name (result_desc), bit_offset, bit_count,
			8 * (u32) buffer_desc->buffer.length));
		goto cleanup;
	}
	...
	status = acpi_ex_prep_common_field_object (
		obj_desc, field_flags, 0, bit_offset, bit_count);
	...
	union acpi_parse_object         *op)
{
	...
	status = acpi_ex_resolve_operands (
		op->common.aml_opcode, ACPI_WALK_OPERANDS, walk_state);
	if (ACPI_FAILURE (status)) {
	...
	union acpi_parse_object         *op)
{
	...
	if (!obj_desc->region.length && (space_id < ACPI_NUM_PREDEFINED_REGIONS)) {
	...
	ACPI_DEBUG_PRINT ((ACPI_DB_EXEC, "RgnObj %p Addr %8.8X%8.8X Len %X\n",
		obj_desc, ACPI_FORMAT_UINT64 (obj_desc->region.address),
		obj_desc->region.length));

	status = acpi_ut_add_address_range (obj_desc->region.space_id,
		obj_desc->region.address, obj_desc->region.length, node);
	...
	union acpi_operand_object       *obj_desc)
{
	...
	case AML_BUFFER_OP:
		...
		status = acpi_ds_build_internal_buffer_obj (
			walk_state, op, length, &obj_desc);
		break;

	case AML_PACKAGE_OP:
	case AML_VARIABLE_PACKAGE_OP:

		status = acpi_ds_build_internal_package_obj (
			walk_state, op, length, &obj_desc);
		break;
	...
	union acpi_parse_object         *op)
{
	...
	while (arg) {
		...
		if (arg->common.aml_opcode == AML_INT_NAMEDFIELD_OP) {
			...
			obj_desc->bank_field.value = (u32) operand_desc->integer.value;
		}
