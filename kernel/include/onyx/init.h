...
enum INIT_LEVEL
{
    ...
    INIT_LEVEL_CORE_DEVICE,
    INIT_LEVEL_CORE_PERCPU_CTOR
};

#define __INIT_ENTRY(func, x)                                                  \
    __attribute__((section(".init.level" #x), used, aligned(1))) static void ( \
        *__PASTE(func, __COUNTER__))(void) = func

#define __INIT_ENTRY_PERCPU(func, x)                                           \
    __attribute__((section(".init.level" #x), used, aligned(1))) static void ( \
        *__PASTE(func, __COUNTER__))(unsigned int) = func
...
#define INIT_LEVEL_CORE_DEVICE_ENTRY(func)         __INIT_ENTRY(func, 8)
#define INIT_LEVEL_CORE_PERCPU_CTOR(func)          __INIT_ENTRY_PERCPU(func, 9)
