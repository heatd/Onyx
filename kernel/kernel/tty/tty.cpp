...
static bool __tty_buf_full(struct tty_buf *buf, unsigned int wpos)
{
    return wpos - buf->rpos == PAGE_SIZE;
}

static bool tty_buf_full(struct tty_buf *buf)
{
    return __tty_buf_full(buf, buf->wpos);
}
