...
#define atomic_inc(atomic) __atomic_add_fetch(atomic, 1, __ATOMIC_RELAXED)
#define atomic_read(atomic) READ_ONCE(*atomic)
...
static noinline void __init check_forking(void)
{
	...
	struct rwlock mt_lock, newmt_lock;

	rwlock_init(&mt_lock);
	rwlock_init(&newmt_lock);
	...
	rw_lock_write(&mt_lock);
	for (i = 0; i <= nr_entries; i++) {
		mas_set_range(&mas, i*10, i*10 + 5);
		mas_store_gfp(&mas, xa_mk_value(i), GFP_KERNEL);
	}

	rw_lock_write(&newmt_lock);
	...
	__mt_destroy(&newmt);
	__mt_destroy(&mt);
	rw_unlock_write(&newmt_lock);
	rw_unlock_write(&mt_lock);
}
...
void __maple_tree_seed()
{
	maple_tree_seed();
}

INIT_LEVEL_CORE_KERNEL_ENTRY(__maple_tree_seed);
