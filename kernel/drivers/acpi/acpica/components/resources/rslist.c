...
acpi_rs_convert_aml_to_resources (
	...
	void                            **context)
{
	struct acpi_resource            **resource_ptr = ACPI_CAST_INDIRECT_PTR (
			  struct acpi_resource, context);
	...
	if (acpi_ut_get_resource_type (aml) ==
		...
		conversion_table = acpi_gbl_get_resource_dispatch[resource_index];
	}
	...
	status = acpi_rs_convert_aml_to_resource (
		resource, aml_resource, conversion_table);
	...
	u8                              *output_buffer)
{
	...
	while (aml < end_aml) {
		...
		if (resource->type == ACPI_RESOURCE_TYPE_SERIAL_BUS) {
			...
			conversion_table = acpi_gbl_set_resource_dispatch[resource->type];
		}
		...
		status = acpi_rs_convert_resource_to_aml (resource,
			ACPI_CAST_PTR (union aml_resource, aml), conversion_table);
		if (ACPI_FAILURE (status)) {
			ACPI_EXCEPTION ((AE_INFO, status,
		...
		status = acpi_ut_validate_resource (
			NULL, ACPI_CAST_PTR (union aml_resource, aml), NULL);
		if (ACPI_FAILURE (status)) {
			return_ACPI_STATUS (status);
		}
