	...
	struct acpi_walk_state          *walk_state)
{
	...
	if (!state) {
		ACPI_ERROR ((AE_INFO, "Result stack is empty! State=%p", walk_state));
	...
	struct acpi_walk_state          *walk_state)
{
	...
	ACPI_DEBUG_PRINT ((ACPI_DB_EXEC, "Obj=%p [%s] State=%p Num=%X Cur=%X\n",
		...
		walk_state, walk_state->result_count, walk_state->current_result));
	...
	u8                              pass_number)
{
	...
	if (method_node) {
		...
		status = acpi_ds_scope_stack_push (
			method_node, ACPI_TYPE_METHOD, walk_state);
		if (ACPI_FAILURE (status)) {
			return_ACPI_STATUS (status);
		}
