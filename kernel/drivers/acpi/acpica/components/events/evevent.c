	...
	void)
{
	...
	status |= acpi_hw_register_read (ACPI_REGISTER_PM1_ENABLE, &fixed_enable);
	if (ACPI_FAILURE (status)) {
		return (int_status);
	}
	...
	for (i = 0; i < ACPI_NUM_FIXED_EVENTS; i++) {
		...
		if ((fixed_status & acpi_gbl_fixed_event_info[i].status_bit_mask) &&
			...
			if (acpi_gbl_global_event_handler) {
				acpi_gbl_global_event_handler (ACPI_EVENT_TYPE_FIXED, NULL,
					 i, acpi_gbl_global_event_handler_context);
			}
