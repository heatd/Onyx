...
char **process_copy_envarg(const char **envarg, bool to_kernel, int *count)
{
	...
	/* Each buffer takes up argc * sizeof pointer + string_size + one extra pointer(to nullptr terminate) */
	size_t nr_args = 0;
...
int process_put_entry_info(struct stack_info *info, char **argv, char **envp)
{
	...
	/* The calling convention passes argv[0] ... argv[?] = nullptr, envp[0] ... envp[?] = nullptr, auxv
