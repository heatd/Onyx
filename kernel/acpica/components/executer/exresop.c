	...
	while (GET_CURRENT_ARG_TYPE (arg_types)) {
		...
		case ACPI_DESC_TYPE_OPERAND:
			...
			if (!acpi_ut_valid_object_type (object_type)) {
				ACPI_ERROR ((AE_INFO,
					"Bad operand object type [0x%X]", object_type));
			...
			if (object_type == (u8) ACPI_TYPE_LOCAL_REFERENCE) {
				...
				default:
					...
					ACPI_ERROR ((AE_INFO,
						...
						obj_desc->reference.class, obj_desc));
		...
		case ARGI_STORE_TARGET:
			...
			status = acpi_ex_check_object_type (
				ACPI_TYPE_LOCAL_REFERENCE, object_type, obj_desc);
		...
		case ARGI_DATAREFOBJ:  /* Store operator only */
			...
			if ((opcode == AML_STORE_OP) &&
				((*stack_ptr)->common.type == ACPI_TYPE_LOCAL_REFERENCE) &&
		...
		case ARGI_STRING:
			...
			status = acpi_ex_convert_to_string (
				obj_desc, stack_ptr, ACPI_IMPLICIT_CONVERT_HEX);
			if (ACPI_FAILURE (status)) {
				if (status == AE_TYPE) {
					ACPI_ERROR ((AE_INFO,
						"Needed [Integer/String/Buffer], found [%s] %p",
		 ...
		 */
		status = acpi_ex_check_object_type (
			type_needed, (*stack_ptr)->common.type, *stack_ptr);
		if (ACPI_FAILURE (status)) {
			return_ACPI_STATUS (status);
		}
...
next_operand:
	...
	ACPI_DUMP_OPERANDS (walk_state->operands,
		acpi_ps_get_opcode_name (opcode), walk_state->num_operands);
