...
database::database(const std::string& path, bool readonly) : path{path}, fd{}, rdonly{readonly}, packages{}
{
	int flags = 0;
...
void database::lock_database()
{
	...
	if(lock < 0)
		throw std::system_error(errno, std::generic_category(), "Failed to lock the database: "
