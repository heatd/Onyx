...
struct acpi_rsconvert_info      acpi_rs_convert_ext_address64[5] =
{
	 ...
	 */
	{ACPI_RSC_MOVE64,   ACPI_RS_OFFSET (data.ext_address64.address.granularity),
...
static struct acpi_rsconvert_info   acpi_rs_convert_io_flags[4] =
{
	...
	{ACPI_RSC_1BITFLAG, ACPI_RS_OFFSET (data.address.info.io.translation_type),
			   AML_OFFSET (address.specific_flags),
			   5}
};
	...
	union aml_resource              *aml)
{
	...
	(void) acpi_rs_convert_aml_to_resource (
		resource, aml, acpi_rs_convert_general_flags);

	/* Get the Type-Specific Flags (Memory and I/O descriptors only) */

	if (resource->data.address.resource_type == ACPI_MEMORY_RANGE) {
		(void) acpi_rs_convert_aml_to_resource (
			resource, aml, acpi_rs_convert_mem_flags);
	}
	...
	struct acpi_resource            *resource)
{
	...
	(void) acpi_rs_convert_resource_to_aml (
		resource, aml, acpi_rs_convert_general_flags);
