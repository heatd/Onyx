		...
		if (walk_state->walk_type & ACPI_WALK_METHOD) {
			...
			if (op->common.aml_opcode != AML_SCOPE_OP) {
				...
				status = acpi_ds_scope_stack_push (
					op->named.node, op->named.node->type, walk_state);
				if (ACPI_FAILURE (status)) {
					return_ACPI_STATUS (status);
				}
			}
	...
	struct acpi_walk_state          *walk_state)
{
	...
	if (op_class == AML_CLASS_UNKNOWN) {
		ACPI_ERROR ((AE_INFO, "Unknown opcode 0x%X", op->common.aml_opcode));
	...
	default:
		...
		case AML_TYPE_METHOD_CALL:
			...
			if ((op->asl.parent) &&
			   ((op->asl.parent->asl.aml_opcode == AML_PACKAGE_OP) ||
				(op->asl.parent->asl.aml_opcode == AML_VARIABLE_PACKAGE_OP))) {
				ACPI_DEBUG_PRINT ((ACPI_DB_DISPATCH,
		...
		case AML_TYPE_CREATE_FIELD:
			 ...
			 */
			namepath = acpi_ns_get_external_pathname (op->common.node);
			status = ae_lookup_init_file_entry (namepath, &obj_desc);
			if (ACPI_SUCCESS (status)) {
				status = acpi_ex_write_data_to_field (obj_desc, op->common.node->object, NULL);
				if (ACPI_FAILURE (status)) {
		...
		case AML_TYPE_CREATE_OBJECT:
			...
			ACPI_DEBUG_PRINT ((ACPI_DB_EXEC,
				...
				op, op->named.value.arg, op->common.parent->common.aml_opcode));
		...
		case AML_TYPE_NAMED_NO_OBJ:
			...
			if (op->common.aml_opcode == AML_REGION_OP) {
				...
				status = acpi_ds_eval_table_region_operands (walk_state, op);
				if (ACPI_FAILURE (status)) {
					break;
				}
			}
				...
				status = acpi_ds_eval_bank_field_operands (walk_state, op);
				if (ACPI_FAILURE (status)) {
					break;
				}
			}
...
cleanup:
	...
	if (walk_state->result_obj) {
		...
		acpi_db_display_result_object (walk_state->result_obj,walk_state);
