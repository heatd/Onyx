	...
	union acpi_operand_object       *match_obj)
{
	...
	case MATCH_MEQ:
		...
		status = acpi_ex_do_logical_op (
			AML_LOGICAL_EQUAL_OP, match_obj, package_obj, &logical_result);
	...
	case MATCH_MLE:
		 ...
		 */
		status = acpi_ex_do_logical_op (
			AML_LOGICAL_LESS_OP, match_obj, package_obj, &logical_result);
		if (ACPI_FAILURE (status)) {
			return (FALSE);
		}
	...
	case MATCH_MLT:
		 ...
		 */
		status = acpi_ex_do_logical_op (
			AML_LOGICAL_GREATER_OP, match_obj, package_obj, &logical_result);
		if (ACPI_FAILURE (status)) {
			return (FALSE);
		}
		break;
