...
public:
    ...
    sys_error(const char* error, int err)
        : std::runtime_error(std::string(error) + ": " + strerror(err))
    {
    }
...
public:
    instance(int fd, const std::string& name)
        : fd{fd}, name{name}, instance_thread(&instance::run, this)
    {
    }
