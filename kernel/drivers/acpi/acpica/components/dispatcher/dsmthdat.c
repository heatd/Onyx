	...
	struct acpi_walk_state          *walk_state)
{
	...
	while ((index < ACPI_METHOD_NUM_ARGS) &&
		...
		status = acpi_ds_method_data_set_value (
			ACPI_REFCLASS_ARG, index, params[index], walk_state);
	...
	struct acpi_walk_state          *walk_state)
{
	...
	if (obj_desc->common.reference_count > 1) {
		status = acpi_ut_copy_iobject_to_iobject (
			obj_desc, &new_obj_desc, walk_state);
		if (ACPI_FAILURE (status)) {
			return_ACPI_STATUS (status);
		}
	}
	 ...
	 */
	status = acpi_ds_method_data_set_value (type, index, new_obj_desc, walk_state);
